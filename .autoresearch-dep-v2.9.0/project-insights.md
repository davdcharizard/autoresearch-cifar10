...
- **torch.compile provides zero speedup on H20 for small models (<5M params) with AMP+channels_last** (EXP-008)
  Evidence: reports/exp-report-008.md § Results — 9-10ms/step with torch.compile vs 9ms baseline; Inductor found no fusion opportunities
  Implication: Skip torch.compile for small-model throughput optimization on H20; AMP+channels_last already minimizes kernel overhead. Compiler gains require larger models or A100-class GPUs.
