...
def train(seed=42, time_budget_s=TIME_BUDGET_S):
    ...
    # First, do warmup steps to ensure torch.compile is fully warmed up
    ...
    # Re-estimate with proper timing using more steps for stability
    model.train()
    step_times = []
    for i in range(10):
        inputs, targets = next(warmup_iter)
    ...
    step_times.sort()
    avg_step_time = step_times[len(step_times) // 2]
