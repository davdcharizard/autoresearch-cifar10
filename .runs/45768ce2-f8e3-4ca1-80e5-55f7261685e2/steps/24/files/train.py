...
class EMAModel:
    def __init__(self, model, decay=0.999):
        ...
        # Also track buffers (batch norm running stats) - only floating point ones
        self.shadow_buffers = {}
    ...
    def update(self, model):
        ...
        for name, buf in model.named_buffers():
            # Only update floating point buffers (skip num_batches_tracked which is Long)
            if name in self.shadow_buffers and buf.is_floating_point():
                self.shadow_buffers[name].mul_(self.decay).add_(buf.data, alpha=1 - self.decay)

    def apply_shadow(self, model):
        """Apply EMA weights to the model (backup current weights first)."""
        self.backup = {}
        for name, param in model.named_parameters():
            if param.requires_grad:
    ...
    def get_shadow_state_dict(self, model):
        """Get a state dict with EMA weights."""
        state = {}
        for name, param in model.named_parameters():
            if param.requires_grad and name in self.shadow:
                state[name] = self.shadow[name].clone()
            else:
                state[name] = param.data.clone()
        for name, buf in model.named_buffers():
            if name in self.shadow_buffers:
                state[name] = self.shadow_buffers[name].clone()
            else:
                state[name] = buf.data.clone()
        return state
...
def train(seed=42, time_budget_s=TIME_BUDGET_S):
    ...
    # Initialize EMA after model reset
    ema = EMAModel(model_raw, decay=EMA_DECAY)
    ...
    while total_training_time < time_budget_s:
        ...
        # Evaluate regular model at end of each epoch using TTA
        ...
        if test_acc > best_acc:
            best_acc = test_acc
            best_state = copy.deepcopy(model_raw_eval.state_dict())
            best_is_ema = False

        # Evaluate EMA model
        ema.apply_shadow(model_raw_eval)
        tta_ema_model = TTAWrapper(model_raw_eval)
        ema_test_loss, ema_test_acc = evaluator.evaluate(tta_ema_model, device)
        ema.restore(model_raw_eval)

        print(f" | ema_acc: {ema_test_acc:.2f}%")

        if ema_test_acc > best_acc:
            best_acc = ema_test_acc
            best_state = ema.get_shadow_state_dict(model_raw_eval)
            best_is_ema = True
            print(f"    ** New best from EMA: {best_acc:.2f}%")
    ...
    print(f"\nBest model is {'EMA' if best_is_ema else 'regular'} with acc: {best_acc:.2f}%")
