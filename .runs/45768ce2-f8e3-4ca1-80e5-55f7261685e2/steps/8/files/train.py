...
def train(seed=42, time_budget_s=TIME_BUDGET_S):
    ...
    # Estimate total steps: ~390 batches/epoch, ~8ms/batch -> ~3.12s/epoch -> ~96 epochs in 300s
    # But with compile overhead, more like 30-35 epochs based on previous runs
    # Use OneCycleLR with estimated total steps
    # From previous experiments, PreActResNet-18 does about 30-35 epochs in 300s
    # Let's estimate ~33 epochs = 33 * 390 = 12870 steps
    estimated_total_steps = 13000
    ...
    while total_training_time < time_budget_s and step < estimated_total_steps:
        epoch += 1
        model.train()
        ...
        for inputs, targets in train_loader:
            ...
            if total_training_time >= time_budget_s or step >= estimated_total_steps:
                break
    ...
    # Pick whichever is better: regular best or EMA best
    if best_ema_acc > best_acc and best_ema_state is not None:
        print(f"Using EMA model: {best_ema_acc:.2f}% > {best_acc:.2f}%")
    ...
    # Final verification
    final_loss, final_acc = evaluator.evaluate(model, device)
