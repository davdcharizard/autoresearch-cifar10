...
# ---------------------------------------------------------------------------
# TTA wrapper - wraps model for test-time augmentation with multi-crop + flips
# ---------------------------------------------------------------------------
class TTAWrapper(nn.Module):
    """Wraps a model to do test-time augmentation (horizontal flip + multi-crop averaging)."""
    ...
    def forward(self, x):
        ...
        # Plus 4 corner shifts (pad and crop differently)
        preds = []
        ...
        # Shifted versions: pad by 2 and take different crops
        pad = 2
        x_padded = F.pad(x, [pad, pad, pad, pad], mode='reflect')
        h, w = x.shape[2], x.shape[3]
        
        # Top-left crop
        preds.append(self.model(x_padded[:, :, 0:h, 0:w]))
        # Top-right crop
        preds.append(self.model(x_padded[:, :, 0:h, 2*pad:2*pad+w]))
        # Bottom-left crop
        preds.append(self.model(x_padded[:, :, 2*pad:2*pad+h, 0:w]))
        # Bottom-right crop
        preds.append(self.model(x_padded[:, :, 2*pad:2*pad+h, 2*pad:2*pad+w]))
        
        # Flipped versions of shifted crops
        preds.append(self.model(torch.flip(x_padded[:, :, 0:h, 0:w], dims=[3])))
        preds.append(self.model(torch.flip(x_padded[:, :, 0:h, 2*pad:2*pad+w], dims=[3])))
        preds.append(self.model(torch.flip(x_padded[:, :, 2*pad:2*pad+h, 0:w], dims=[3])))
        preds.append(self.model(torch.flip(x_padded[:, :, 2*pad:2*pad+h, 2*pad:2*pad+w], dims=[3])))
        
        return sum(preds) / len(preds)


# Simple TTA for faster per-epoch evaluation
class SimpleTTAWrapper(nn.Module):
    """Simple TTA with just horizontal flip for faster per-epoch evaluation."""
    def __init__(self, model):
        super().__init__()
        self.model = model
...
def train(seed=42, time_budget_s=TIME_BUDGET_S):
    ...
    while total_training_time < time_budget_s:
        ...
        # Evaluate at end of each epoch using simple TTA (fast)
        model_raw_eval = model._orig_mod if hasattr(model, '_orig_mod') else model
        simple_tta_model = SimpleTTAWrapper(model_raw_eval)
        test_loss, test_acc = evaluator.evaluate(simple_tta_model, device)
    ...
    # Load the best checkpoint and wrap with full TTA (multi-crop + flips)
    model_raw = model._orig_mod if hasattr(model, '_orig_mod') else model
    if best_state is not None:
        model_raw.load_state_dict(best_state)

    # Return the full TTA-wrapped model for final evaluation
