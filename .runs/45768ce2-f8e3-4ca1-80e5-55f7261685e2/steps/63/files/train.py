...
def train(seed=42, time_budget_s=TIME_BUDGET_S):
    ...
    # Try to compile the model with reduce-overhead mode for faster execution
    try:
        model = torch.compile(model, mode="reduce-overhead")
        print("Model compiled successfully (reduce-overhead mode)")
    except Exception as e:
        print(f"torch.compile reduce-overhead failed: {e}, trying default...")
        try:
