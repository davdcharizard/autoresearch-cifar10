...
def train(seed=42, time_budget_s=TIME_BUDGET_S):
    ...
    # First, do warmup steps to let torch.compile settle
    print("Warming up and estimating step time...")
    model.train()
    ...
    for i in range(15):
        ...
        warmup_steps_count += 1

    # Time steps for estimation
    step_times = []
    for i in range(10):
        inputs, targets = next(warmup_iter)
        inputs = inputs.to(device, non_blocking=True)
        ...
        # Apply Mixup during timing too for accurate estimation
    ...
    step_times.sort()
    avg_step_time = step_times[len(step_times) // 2]  # median
    ...
    print(f"Median step time: {avg_step_time*1000:.1f}ms")
    print(f"Estimated total steps: {estimated_total_steps}, epochs: {estimated_epochs:.1f}")
    print(f"Warmup steps done: {warmup_steps_count}")

    # Don't reset model - continue training from warmup state
    # Account for warmup steps in the scheduler
    total_scheduled_steps = estimated_total_steps + warmup_steps_count
    
    # Warmup for 5% of training then cosine decay
    warmup_lr_steps = min(500, total_scheduled_steps // 20)
    
    def lr_lambda(step):
        if step < warmup_lr_steps:
            return (step + 1) / warmup_lr_steps
        progress = (step - warmup_lr_steps) / max(1, total_scheduled_steps - warmup_lr_steps)
        progress = min(progress, 1.0)
        return 0.5 * (1 + math.cos(math.pi * progress))

    # Re-create optimizer to reset momentum buffers but keep model weights
    optimizer = optim.SGD(
        model.parameters(), lr=LR, momentum=MOMENTUM,
        weight_decay=WEIGHT_DECAY, nesterov=True
    )
    ...
    # Step scheduler for warmup steps already done
    ...
    epoch = 0
    step = warmup_steps_count  # Continue step count from warmup
    best_acc = 0.0
    best_state = None
    ...
    while total_training_time < time_budget_s:
        ...
        for inputs, targets in train_loader:
            ...
            debiased = smooth_train_loss / (1 - ema_beta ** (step - warmup_steps_count))
