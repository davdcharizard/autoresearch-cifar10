...
def train(seed=42, time_budget_s=TIME_BUDGET_S):
    ...
    # Set T_0 so that we get exactly 2 complete cycles: T_0 + T_0*T_mult = total
    # With T_mult=2: T_0 + 2*T_0 = 3*T_0 = total_steps_after_warmup
    # So T_0 = total_steps_after_warmup / 3
    effective_steps = estimated_total_steps - warmup_steps
    # Use 3 cycles: T_0, 2*T_0, 4*T_0 = 7*T_0 total
    # Or 2 cycles: T_0, 2*T_0 = 3*T_0 total
    # Let's use T_mult=2, and compute T_0 for 2 full cycles (3*T_0 = effective_steps)
    T_0_steps = max(effective_steps // 3, 100)
    T_mult = 2
    ...
    def lr_lambda(step):
        ...
        # Compute which cycle we're in and position within it
        # Cycle lengths: T_0, T_0*T_mult, T_0*T_mult^2, ...
        current_T = T_0_steps
        cycle_start = 0
        while cycle_start + current_T <= adjusted_step:
            cycle_start += current_T
            current_T = int(current_T * T_mult)
        
        # Position within current cycle [0, 1]
        progress = (adjusted_step - cycle_start) / max(1, current_T)
        progress = min(progress, 1.0)
        
        # Cosine decay within cycle, with minimum LR factor
        return max(0.5 * (1 + math.cos(math.pi * progress)), 1e-5)
    ...
    print(f"Warm restart T_0: {T_0_steps} steps, T_mult: {T_mult}")
