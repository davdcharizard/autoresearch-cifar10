...
# ---------------------------------------------------------------------------
# PreAct ResNet with configurable blocks
# ---------------------------------------------------------------------------
...
def PreActResNet18_deeper(num_classes=10):
    # [3,3,3,3] instead of [2,2,2,2] - adds 4 more blocks for more capacity
    return PreActResNet(PreActBlock, [3, 3, 3, 3], num_classes=num_classes)
...
def train(seed=42, time_budget_s=TIME_BUDGET_S):
    ...
    model = PreActResNet18_deeper(NUM_CLASSES).to(device)
    num_params = sum(p.numel() for p in model.parameters())
    print(f"PreActResNet [3,3,3,3] | params: {num_params:,}")
