...
def train(seed=42, time_budget_s=TIME_BUDGET_S):
    ...
    # First, do warmup steps to trigger compilation
    ...
    # Time a full epoch for accurate estimation
    print("Timing a full epoch for schedule calibration...")
    model.train()
    steps_per_epoch = len(train_loader)
    
    # Time remaining warmup steps to complete the first epoch
    # We've already done 10 steps from warmup_iter, so we need to continue
    epoch_step_times = []
    ...
    for inputs, targets in train_loader:
        ...
        optimizer.step()
        torch.cuda.synchronize()
        epoch_step_times.append(time.time() - t0)
        step_count_in_epoch += 1
        if step_count_in_epoch >= 30:  # Time 30 steps for robust estimate
            break
    ...
    print(f"Avg step time: {avg_step_time*1000:.1f}ms (from {len(epoch_step_times)} steps)")
