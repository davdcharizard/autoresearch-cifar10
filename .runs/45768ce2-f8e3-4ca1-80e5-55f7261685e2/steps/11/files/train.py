...
class ResNeXtBlock(nn.Module):
    ...
    def __init__(self, in_channels, mid_channels, cardinality, stride=1):
        super().__init__()
        D = mid_channels * cardinality  # group conv width
        out_channels = mid_channels * self.expansion * cardinality
        ...
        self.conv3 = nn.Conv2d(D, out_channels, 1, bias=False)
...
# ---------------------------------------------------------------------------
# PreAct ResNet-18 (proven best architecture from memory)
# ---------------------------------------------------------------------------
...
def train(seed=42, time_budget_s=TIME_BUDGET_S):
    ...
    for i in range(10):
        ...
        if i >= 3:  # skip first few for JIT warmup
            warmup_times.append(time.time() - t_start if i == 3 else 0)
