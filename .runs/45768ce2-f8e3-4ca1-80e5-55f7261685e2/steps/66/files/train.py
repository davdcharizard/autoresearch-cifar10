...
def train(seed=42, time_budget_s=TIME_BUDGET_S):
    ...
    # Loss function with label smoothing (will be recreated with annealed smoothing)
    ...
    # Function to compute annealed label smoothing
    def get_label_smoothing(step):
        """Anneal label smoothing from LABEL_SMOOTHING to 0.0 following cosine schedule."""
        if step < warmup_steps:
            return LABEL_SMOOTHING
        progress = (step - warmup_steps) / max(1, estimated_total_steps - warmup_steps)
        progress = min(progress, 1.0)
        # Cosine anneal from LABEL_SMOOTHING to 0.0
        return LABEL_SMOOTHING * 0.5 * (1 + math.cos(math.pi * progress))
    ...
    # Pre-create criterion objects for different smoothing values to avoid overhead
    # We'll update the criterion periodically (every epoch)
    current_smoothing = LABEL_SMOOTHING
    criterion = nn.CrossEntropyLoss(label_smoothing=current_smoothing)

    while total_training_time < time_budget_s:
        epoch += 1
        model.train()

        # Update label smoothing at start of each epoch
        new_smoothing = get_label_smoothing(step)
        if abs(new_smoothing - current_smoothing) > 0.001:
            current_smoothing = new_smoothing
            criterion = nn.CrossEntropyLoss(label_smoothing=current_smoothing)
        ...
        for inputs, targets in train_loader:
            ...
            if step % 100 == 0:
                ls = get_label_smoothing(step)
                print(
                    f"\rstep {step:05d} ep {epoch} ({pct_done:.1f}%) | loss: {debiased:.4f} | lr: {lr:.5f} | ls: {ls:.4f} | rem: {remaining:.0f}s    ",
                    end="", flush=True,
                )
