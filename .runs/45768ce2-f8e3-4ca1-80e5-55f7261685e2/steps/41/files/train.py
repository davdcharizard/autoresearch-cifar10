...
BATCH_SIZE = 128
LR = 0.1
MIN_LR = 1e-4  # minimum LR for cosine schedule
MOMENTUM = 0.9
WEIGHT_DECAY = 5e-4
CUTOUT_LENGTH = 16
MIXUP_ALPHA = 0.2
...
def train(seed=42, time_budget_s=TIME_BUDGET_S):
    ...
    # Time a full epoch worth of steps for more accurate estimation
    model.train()
    step_times = []
    for i in range(5):
        inputs, targets = next(warmup_iter)
    ...
    # Warmup for 5% of training then cosine decay to MIN_LR
    warmup_steps = min(500, estimated_total_steps // 20)
    min_lr_ratio = MIN_LR / LR  # ratio of min LR to max LR
