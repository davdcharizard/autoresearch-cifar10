...
def train(seed=42, time_budget_s=TIME_BUDGET_S):
    ...
    # From previous experiments: ~390 steps/epoch, ~30 epochs in budget
    # Be conservative to ensure LR reaches near 0
    steps_per_epoch = len(train_loader)
    # Estimate based on previous runs: about 30-35 epochs in 300s
    estimated_epochs = 30
    ...
    while total_training_time < time_budget_s:
        ...
        print(
            f"\n  eval ep {epoch:3d} | acc: {test_acc:.2f}% (best: {best_acc:.2f}%) | "
            f"ema_acc: {ema_test_acc:.2f}% (best: {best_ema_acc:.2f}%)"
        )

        if epoch == 1:
            gc.collect()

    # Load best model (pick whichever is better: regular or EMA)
    if best_ema_acc >= best_acc and best_ema_state is not None:
        model.load_state_dict(best_ema_state)
        final_best = best_ema_acc
