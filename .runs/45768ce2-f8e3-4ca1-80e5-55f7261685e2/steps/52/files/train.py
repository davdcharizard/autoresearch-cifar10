...
def train(seed=42, time_budget_s=TIME_BUDGET_S):
    ...
    # First, do a few warmup steps to estimate step time (compilation warmup)
    print("Estimating step time...")
    model.train()
    warmup_iter = iter(train_loader)
    # Do 10 steps for compilation warmup (these are NOT timed)
    for i in range(10):
    ...
    # Time 5 more steps for estimation
    model.train()
    step_times = []
    for i in range(5):
        inputs, targets = next(warmup_iter)
