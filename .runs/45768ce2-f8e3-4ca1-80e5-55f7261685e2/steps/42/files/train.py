...
class EMAModel:
    ...
    def __init__(self, model, decay=0.999):
        ...
        # Store shadow copies of floating-point buffers (BN running stats)
        for name, buf in model.named_buffers():
            if buf.is_floating_point():
                self.shadow['buffer_' + name] = buf.data.clone()
    ...
    def update(self, model):
        ...
        for name, buf in model.named_buffers():
            if buf.is_floating_point():
                key = 'buffer_' + name
                self.shadow[key].lerp_(buf.data, 1.0 - decay)
    ...
    def apply_shadow(self, model):
        ...
        for name, buf in model.named_buffers():
            if buf.is_floating_point():
                ...
                buf.data.copy_(self.shadow[key])
        ...
        for name, buf in model.named_buffers():
            if buf.is_floating_point():
                ...
                if key in self.backup:
                    buf.data.copy_(self.backup[key])
        self.backup = {}

    def get_state_dict(self):
        """Get the EMA shadow state dict."""
...
def train(seed=42, time_budget_s=TIME_BUDGET_S):
    ...
    while total_training_time < time_budget_s:
        ...
        tta_model = TTAWrapper(model_raw)
        ...
        # Evaluate EMA model
        ema.apply_shadow(model_raw)
        tta_ema_model = TTAWrapper(model_raw)
        ema_test_loss, ema_test_acc = evaluator.evaluate(tta_ema_model, device)
        
        if ema_test_acc > best_ema_acc:
            best_ema_acc = ema_test_acc
            best_ema_state = copy.deepcopy(model_raw.state_dict())
        
        ema.restore(model_raw)

        overall_best = max(best_acc, best_ema_acc)

        print(
            f"\n  eval ep {epoch:3d} | test_acc: {test_acc:.2f}% | ema_acc: {ema_test_acc:.2f}% | best: {overall_best:.2f}%"
        )

        if epoch == 1:
            gc.collect()

    # Load the best checkpoint (regular or EMA, whichever is better)
    if best_ema_acc > best_acc and best_ema_state is not None:
        model_raw.load_state_dict(best_ema_state)
        final_best_acc = best_ema_acc
        print(f"Using EMA model: {best_ema_acc:.2f}%")
    elif best_state is not None:
        model_raw.load_state_dict(best_state)
        final_best_acc = best_acc
        print(f"Using regular model: {best_acc:.2f}%")
    else:
        final_best_acc = max(best_acc, best_ema_acc)
