...
MIXUP_ALPHA = 0.2
LABEL_SMOOTHING = 0.1
WIDTH_MULT = 1.25  # Wider PreActResNet-18
evaluator = Eval()
...
def PreActResNet18(num_classes=10, width_mult=1.0):
    return PreActResNet(PreActBlock, [2, 2, 2, 2], num_classes=num_classes, width_mult=width_mult)
...
def train(seed=42, time_budget_s=TIME_BUDGET_S):
    ...
    model = PreActResNet18(NUM_CLASSES, width_mult=WIDTH_MULT).to(device)
    num_params = sum(p.numel() for p in model.parameters())
    print(f"PreActResNet-18 (width_mult={WIDTH_MULT}) | params: {num_params:,}")
