...
# ---------------------------------------------------------------------------
# TTA wrapper - wraps model for test-time augmentation (horizontal flip)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Multi-crop TTA wrapper for final evaluation
# ---------------------------------------------------------------------------
class MultiCropTTAWrapper(nn.Module):
    """More aggressive TTA with horizontal flips and shifted crops."""
    def __init__(self, model, shifts=2):
        super().__init__()
        self.model = model
        self.shifts = shifts
    ...
    def forward(self, x):
        ...
        predictions.append(self.model(torch.flip(x, dims=[3])))
        
        # Shifted versions (pad and crop)
        s = self.shifts
        for dy, dx in [(s, 0), (-s, 0), (0, s), (0, -s)]:
            shifted = torch.roll(x, shifts=(dy, dx), dims=(2, 3))
            predictions.append(self.model(shifted))
            predictions.append(self.model(torch.flip(shifted, dims=[3])))
...
def train(seed=42, time_budget_s=TIME_BUDGET_S):
    ...
    while total_training_time < time_budget_s:
        ...
        # Evaluate at end of each epoch using simple flip TTA
    ...
    # Load the best checkpoint and wrap with multi-crop TTA
    model_raw = model._orig_mod if hasattr(model, '_orig_mod') else model
    if best_state is not None:
        model_raw.load_state_dict(best_state)

    # Return the multi-crop TTA-wrapped model for final evaluation
    final_model = MultiCropTTAWrapper(model_raw, shifts=2)
