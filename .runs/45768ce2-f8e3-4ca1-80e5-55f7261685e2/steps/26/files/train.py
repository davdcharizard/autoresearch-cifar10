...
class SEPreActBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, se_reduction=16):
        ...
        self.se = SELayer(planes, reduction=se_reduction)
...
class SEPreActResNet(nn.Module):
...
def SEPreActResNet18(num_classes=10):
    return SEPreActResNet(SEPreActBlock, [2, 2, 2, 2], num_classes=num_classes)
...
def train(seed=42, time_budget_s=TIME_BUDGET_S):
    ...
    model = SEPreActResNet18(NUM_CLASSES).to(device)
    num_params = sum(p.numel() for p in model.parameters())
    print(f"SE-PreActResNet-18 | params: {num_params:,}")
