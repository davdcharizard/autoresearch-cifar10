...
# ---------------------------------------------------------------------------
def train_single_run(seed, time_budget_s, device, train_set, run_id=0):
    """Train a single model within the given time budget."""
    ...
    print(f"Run {run_id}: PreActResNet-18 | params: {num_params:,}")

    # Try to compile the model
    try:
        model = torch.compile(model)
        print(f"Run {run_id}: Model compiled successfully")
    except Exception as e:
        print(f"Run {run_id}: torch.compile failed: {e}")
    ...
    print(f"Run {run_id}: Estimating step time...")
    model.train()
    ...
    print(f"Run {run_id}: Avg step time: {avg_step_time*1000:.1f}ms")
    print(f"Run {run_id}: Estimated total steps: {estimated_total_steps}, epochs: {estimated_epochs:.1f}")
    ...
    print(f"Run {run_id}: Time budget: {time_budget_s}s")
    print(f"Run {run_id}: Batches per epoch: {steps_per_epoch}")
    ...
    while total_training_time < time_budget_s:
        ...
        for inputs, targets in train_loader:
            ...
            if step % 200 == 0:
                print(
                    f"\rRun {run_id} step {step:05d} ep {epoch} ({pct_done:.1f}%) | loss: {debiased:.4f} | lr: {lr:.5f} | rem: {remaining:.0f}s    ",
                    end="", flush=True,
                )
        ...
        print(
            f"\n  Run {run_id} eval ep {epoch:3d} | test_acc: {test_acc:.2f}% | best: {best_acc:.2f}%"
        )

        if epoch == 1:
            gc.collect()

    return best_acc, best_state, total_training_time, num_params, epoch, step
...
def train(seed=42, time_budget_s=TIME_BUDGET_S):
    ...
    # Run the full training with the entire budget
    best_acc, best_state, total_training_time, num_params, num_epochs, num_steps = \
        train_single_run(seed, time_budget_s, device, train_set, run_id=0)
    ...
    final_model = TTAWrapper(model)

    t_end = time.time()
    startup_time = t_start - t_start  # minimal
    peak_vram_mb = (
