...
# ---------------------------------------------------------------------------
# Random padding crop - varies the padding for scale diversity
# ---------------------------------------------------------------------------
class RandomPaddingCrop:
    """RandomCrop with random padding between min_pad and max_pad for scale diversity."""
    def __init__(self, size=32, min_pad=2, max_pad=6):
        self.size = size
    ...
    def __call__(self, img):
        # img is a PIL Image
        import random
        pad = random.randint(self.min_pad, self.max_pad)
        # Use transforms functional
        from torchvision.transforms import functional as TF
        img = TF.pad(img, pad, padding_mode='reflect')
        # Random crop back to original size
        i, j, h, w = transforms.RandomCrop.get_params(img, (self.size, self.size))
...
def train(seed=42, time_budget_s=TIME_BUDGET_S):
    ...
    train_tf = transforms.Compose([
        RandomPaddingCrop(size=32, min_pad=2, max_pad=6),
