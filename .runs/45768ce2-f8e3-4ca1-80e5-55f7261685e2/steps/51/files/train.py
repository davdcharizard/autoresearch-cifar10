...
MIXUP_ALPHA = 0.2
LABEL_SMOOTHING = 0.1
NUM_SNAPSHOTS = 5  # Number of late-stage checkpoints to ensemble
evaluator = Eval()
...
# ---------------------------------------------------------------------------
# Snapshot Ensemble wrapper - averages predictions from multiple checkpoints
# ---------------------------------------------------------------------------
class SnapshotEnsembleWrapper(nn.Module):
    """Wraps multiple model states and averages their TTA predictions."""
    def __init__(self, base_model, state_dicts):
        super().__init__()
        self.base_model = base_model
        self.state_dicts = state_dicts
    
    def forward(self, x):
        # Also compute flipped version
        ...
        return total_out / len(self.state_dicts)
...
def train(seed=42, time_budget_s=TIME_BUDGET_S):
    ...
    # For snapshot ensemble: keep recent states from low-LR phase
    snapshot_states = []  # List of (acc, state_dict) tuples
    snapshot_threshold = 0.7  # Start saving snapshots after 70% of estimated training
    ...
    while total_training_time < time_budget_s:
        ...
        # Track best single checkpoint
        if test_acc > best_acc:
            best_acc = test_acc
            model_raw = model._orig_mod if hasattr(model, '_orig_mod') else model
            best_state = copy.deepcopy(model_raw.state_dict())

        # Save snapshots in the low-LR phase (last 30% of training)
        progress = step / max(1, estimated_total_steps)
        if progress >= snapshot_threshold:
            model_raw_snap = model._orig_mod if hasattr(model, '_orig_mod') else model
            sd = copy.deepcopy(model_raw_snap.state_dict())
            snapshot_states.append((test_acc, sd))
            # Keep only the best NUM_SNAPSHOTS snapshots
            if len(snapshot_states) > NUM_SNAPSHOTS:
                # Remove the one with lowest accuracy
                snapshot_states.sort(key=lambda x: x[0], reverse=True)
                snapshot_states = snapshot_states[:NUM_SNAPSHOTS]

        print(
            f"\n  eval ep {epoch:3d} | test_loss: {test_loss:.4f} | test_acc: {test_acc:.2f}% | best: {best_acc:.2f}% | snapshots: {len(snapshot_states)}"
        )
    ...
    # ---------------------------------------------------------------------------
    # Choose best final model: single best vs snapshot ensemble
    # ---------------------------------------------------------------------------
    model_raw = model._orig_mod if hasattr(model, '_orig_mod') else model
    
    if len(snapshot_states) >= 3:
        # Try snapshot ensemble
        snapshot_sds = [sd for _, sd in snapshot_states]
        ensemble_model = SnapshotEnsembleWrapper(model_raw, snapshot_sds)
        ensemble_model.eval()
        ensemble_loss, ensemble_acc = evaluator.evaluate(ensemble_model, device)
        print(f"  Snapshot ensemble ({len(snapshot_sds)} models): {ensemble_acc:.2f}% vs best single: {best_acc:.2f}%")
        
        if ensemble_acc > best_acc:
            print(f"  Using snapshot ensemble (better by {ensemble_acc - best_acc:.2f}%)")
            final_model = ensemble_model
            best_acc = ensemble_acc
        else:
            print(f"  Using best single checkpoint")
