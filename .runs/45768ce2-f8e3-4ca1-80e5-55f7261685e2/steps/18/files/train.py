...
MIXUP_ALPHA = 0.2
LABEL_SMOOTHING = 0.1
DROP_PATH_RATE = 0.1  # Stochastic depth max drop rate
evaluator = Eval()
...
# ---------------------------------------------------------------------------
# PreAct ResNet-18 with Stochastic Depth
# ---------------------------------------------------------------------------
class PreActBlock(nn.Module):
    expansion = 1

    def __init__(self, in_planes, planes, stride=1, drop_path_rate=0.0):
        super().__init__()
...
class PreActResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, width_mult=1, drop_path_rate=0.0):
        super().__init__()
        self.in_planes = 64
        ...
        # Calculate total number of blocks for linear drop path schedule
        total_blocks = sum(num_blocks)
        block_idx = 0
        
        # Build layers with linearly increasing drop path rate
        layers1 = []
        strides = [1] + [1] * (num_blocks[0] - 1)
        for s in strides:
            dp_rate = drop_path_rate * block_idx / max(total_blocks - 1, 1)
            layers1.append(block(self.in_planes, widths[0], s, drop_path_rate=dp_rate))
            self.in_planes = widths[0] * block.expansion
            block_idx += 1
        ...
        for s in strides:
            dp_rate = drop_path_rate * block_idx / max(total_blocks - 1, 1)
            layers2.append(block(self.in_planes, widths[1], s, drop_path_rate=dp_rate))
            self.in_planes = widths[1] * block.expansion
            block_idx += 1
        ...
        for s in strides:
            dp_rate = drop_path_rate * block_idx / max(total_blocks - 1, 1)
            layers3.append(block(self.in_planes, widths[2], s, drop_path_rate=dp_rate))
            self.in_planes = widths[2] * block.expansion
            block_idx += 1
        self.layer3 = nn.Sequential(*layers3)
        
        layers4 = []
        strides = [2] + [1] * (num_blocks[3] - 1)
        for s in strides:
            dp_rate = drop_path_rate * block_idx / max(total_blocks - 1, 1)
            layers4.append(block(self.in_planes, widths[3], s, drop_path_rate=dp_rate))
            self.in_planes = widths[3] * block.expansion
            block_idx += 1
...
def PreActResNet18(num_classes=10, drop_path_rate=0.0):
    return PreActResNet(PreActBlock, [2, 2, 2, 2], num_classes=num_classes, drop_path_rate=drop_path_rate)
...
def train(seed=42, time_budget_s=TIME_BUDGET_S):
    ...
    model = PreActResNet18(NUM_CLASSES, drop_path_rate=DROP_PATH_RATE).to(device)
    num_params = sum(p.numel() for p in model.parameters())
    print(f"PreActResNet-18 (with DropPath={DROP_PATH_RATE}) | params: {num_params:,}")
