...
MIXUP_ALPHA = 0.2
LABEL_SMOOTHING = 0.1
MIXUP_CUTOFF = 0.8  # Stop mixup after 80% of training
evaluator = Eval()
...
def train(seed=42, time_budget_s=TIME_BUDGET_S):
    ...
    # Loss function without label smoothing for clean fine-tuning phase
    ...
    # Compute mixup cutoff step
    mixup_cutoff_step = int(estimated_total_steps * MIXUP_CUTOFF)
    print(f"Mixup cutoff at step {mixup_cutoff_step} ({MIXUP_CUTOFF*100:.0f}% of training)")
    ...
    while total_training_time < time_budget_s:
        ...
        for inputs, targets in train_loader:
            ...
            # Apply Mixup only during first MIXUP_CUTOFF fraction of training
            use_mixup = (step < mixup_cutoff_step)
            ...
            if step % 100 == 0:
                ...
                print(
                    f"\rstep {step:05d} ep {epoch} ({pct_done:.1f}%) | loss: {debiased:.4f} | lr: {lr:.5f} | mixup: {mixup_status} | rem: {remaining:.0f}s    ",
                    end="", flush=True,
                )
