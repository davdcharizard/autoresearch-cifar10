...
class EMAModel:
    ...
    def update(self, model):
        ...
        for name, buf in model.named_buffers():
            if name in self.shadow_buffers:
                self.shadow_buffers[name].copy_(buf.data)
    ...
    def get_state_dict(self, model):
        """Get state dict with EMA weights applied."""
        # Save originals
        orig_params = {}
        for name, param in model.named_parameters():
            if name in self.shadow:
        ...
        for name, buf in model.named_buffers():
            if name in self.shadow_buffers:
                orig_buffers[name] = buf.data.clone()
                buf.data.copy_(self.shadow_buffers[name])
        ...
        for name, buf in model.named_buffers():
            if name in orig_buffers:
                buf.data.copy_(orig_buffers[name])
...
def train(seed=42, time_budget_s=TIME_BUDGET_S):
    ...
    while total_training_time < time_budget_s:
        ...
        # Evaluate EMA model
        ema_state = ema.get_state_dict(model_raw_eval)
        # Save original state
        orig_state = copy.deepcopy(model_raw_eval.state_dict())
        # Load EMA state
        model_raw_eval.load_state_dict(ema_state)
        tta_ema_model = TTAWrapper(model_raw_eval)
        ema_test_loss, ema_test_acc = evaluator.evaluate(tta_ema_model, device)
        # Restore original state
        model_raw_eval.load_state_dict(orig_state)
        ...
        print(
            f"  EMA  ep {epoch:3d} | test_loss: {ema_test_loss:.4f} | test_acc: {ema_test_acc:.2f}% | best: {best_acc:.2f}% {'(EMA)' if best_is_ema else ''}"
        )
