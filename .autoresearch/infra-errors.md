...
- **Local foreground training session can return `-1` before final metrics** (EXP-042)
  Error: `exec session returned code -1`; Python worker vanished and `run.log` stopped before `best_test_acc`.
  Resolution: classify as crash if no final metric after retry; verify PIDs by `/proc/<pid>/cwd` before killing, since other runs may share `uv run train.py`.

- **Detached background launches are reaped with empty logs in this shell** (EXP-042)
  Error: `nohup ... & echo $!` and subshell background launches exited immediately with 0-byte logs.
  Resolution: use foreground launches for local CIFAR runs; do not rely on detached background jobs unless survival is proven first.
