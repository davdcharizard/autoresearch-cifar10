# Infrastructure Errors

<!-- Cross-loop record of environment, deploy, and job-platform errors and their resolutions.
     Cross-goal by nature (shared machine / cluster / image). Approach-level failures belong in
     goals/{slug}/03-experiment-learnings.md § Failed Approaches — not here.

     Severity tiers:
       Critical  — Blocks execution entirely or causes data loss.
       Important — Causes experiment failure but has a known workaround.
       Warning   — Non-fatal. Degrades performance or produces unexpected behavior.

     Entry format (3-line, budget-strict; HARD CEILING ~500 chars per bullet):

       - **{Concise error — ≤150 chars}** ({source ref})
         Error: `{error message or traceback summary — one line}`
         Resolution: {step-by-step fix — ≤2 lines. Link to issue or commit if applicable.}

     De-duplication: before appending, scan for a matching error signature. If found, extend
     the source-ref list — do not duplicate. -->

## Infrastructure Errors

### Critical

<!-- Example:
       - **OOM on 4-GPU DDP with batch size > 64** (EXP-002)
         Error: `CUDA out of memory. Tried to allocate 2.40 GiB`
         Resolution: reduce per-GPU batch size to 32 or enable gradient checkpointing via
         `model.gradient_checkpointing_enable()`; if still OOM, switch to 8-GPU resource queue. -->

### Important

<!-- Example:
       - **Image pull timeout on ml-platform queue** (EXP-005)
         Error: `ErrImagePull: context deadline exceeded`
         Resolution: default pull timeout (5 min) too short; add `imagePullPolicy: IfNotPresent`
         and pre-cache image on target node pool, or use smaller base image. -->

### Warning

- **Diagnostic forward hooks must return `None` unless replacing output intentionally** (EXP-038)
  Error: `RuntimeError: mat1 and mat2 shapes cannot be multiplied` after a capture hook returned its stored tensor.
  Resolution: assign captures inside a named hook with no return; assert hooked and unhooked output shapes before trusting diagnostics.

- **Clone pinned/shared DataLoader RNG states before CPU replay** (EXP-033)
  Error: `exit 139 in torch.set_rng_state` or `RuntimeError: Invalid mt19937 state`
  Resolution: clone the batched state into contiguous parent memory, then clone each row before `set_rng_state`; never replay a pinned/shared or sliced state view directly.

- **Deterministic CUDA controllers need cuBLAS workspace config before process start** (EXP-020)
  Error: `RuntimeError: Deterministic behavior ... uses CuBLAS ... set CUBLAS_WORKSPACE_CONFIG`
  Resolution: launch with `CUBLAS_WORKSPACE_CONFIG=:4096:8`; setting deterministic algorithms inside Python alone is insufficient.

- **Path-launched ignored controllers cannot import root modules implicitly** (EXP-016)
  Error: `ModuleNotFoundError: No module named 'train'`
  Resolution: resolve the project root from `__file__` and prepend it to `sys.path` before root-module imports; verify with `--help` before GPU work.

- **First fresh inference subprocess can remain device-cold after local warmups** (EXP-015)
  Error: `accepted inference CV=6.282%; first process 2.2608ms versus later 1.9689-1.9855ms`
  Resolution: predeclare one unscored device-conditioning subprocess before randomized trials, or permit one unchanged full-protocol infrastructure retry.

<!-- Example:
       - **WandB sync delay causes missing early metrics** (EXP-003)
         Error: no error — first 50 steps of metrics missing from WandB dashboard
         Resolution: call `wandb.log(..., commit=True)` at step 0 and every 10 steps during warmup. -->
