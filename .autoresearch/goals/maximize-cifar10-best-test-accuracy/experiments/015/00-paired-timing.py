...
def model_class(name):
    return candidate.ResNet if name == "candidate" else load_baseline()["ResNet"]


def make_inputs():
    generator = torch.Generator().manual_seed(20260806)
    inputs = torch.randn(BATCH_SIZE, 3, 32, 32, generator=generator).pin_memory()
    hard_targets = torch.randint(
        0, 10, (BATCH_SIZE,), generator=generator
    ).pin_memory()
...
def train_worker(name):
    ...
    for step in range(WARMUP_STEPS + TIMED_STEPS):
        ...
        batch = inputs.cuda(non_blocking=True)
        target_batch = targets.cuda(non_blocking=True)
        optimizer.zero_grad()
...
def inference_worker(name):
    ...
    with torch.inference_mode():
        for step in range(WARMUP_STEPS + TIMED_STEPS):
            start = time.perf_counter()
            model(inputs.cuda(non_blocking=True))
            torch.cuda.synchronize()
            elapsed = time.perf_counter() - start
            if step >= WARMUP_STEPS:
                durations.append(elapsed)
...
def orchestrate():
    ...
    for trial in range(TRIALS):
        ...
        for name in order:
            ...
            training[name].append(result)
            print(f"train trial {trial + 1} {name}: {result}", flush=True)

    accepted_means = [result["mean"] for result in training["accepted"]]
    candidate_means = [result["mean"] for result in training["candidate"]]
    ...
    accepted_peak = statistics.median(
        result["peak_mb"] for result in training["accepted"]
    )
    candidate_peak = statistics.median(
        result["peak_mb"] for result in training["candidate"]
    )
    training_summary = {
        ...
        "p95_ratio": statistics.median(
            result["p95"] for result in training["candidate"]
        )
        / statistics.median(result["p95"] for result in training["accepted"]),
    ...
    assert training_summary["ratio"] <= 1.01
    assert training_summary["projected_steps"] >= 26_629
    assert training_summary["p95_ratio"] <= 1.02
    assert training_summary["candidate_peak_mb"] < 650
    assert training_summary["peak_delta_mb"] <= 16
    assert inference_summary["accepted_cv"] <= 0.02
    assert inference_summary["candidate_cv"] <= 0.02
    assert inference_summary["ratio"] <= 1.01
    assert projected_total < 540
    print("EXP015 TIMING GATE: PASS")
