...
def orchestrate():
    ...
    assert ratio <= 1.025
    assert projected_steps >= 26_242
    assert training_summary["p95_ratio"] <= 1.07
    assert training_summary["candidate_peak_mb"] < 620
    assert inference_summary["accepted_cv"] <= 0.02
    assert inference_summary["candidate_cv"] <= 0.02
    assert inference_summary["ratio"] <= 1.05
    assert projected_total < 540
    print("EXP014 TIMING GATE: PASS")
