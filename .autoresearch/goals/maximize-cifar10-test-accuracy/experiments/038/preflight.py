...
def optimizer_for(module, model, candidate):
    if candidate:
        ...
        classifier = [
            parameter
            for name, parameter in model.named_parameters()
            if parameter.requires_grad and name == "fc.weight"
        ]
        no_decay = [
            parameter
            for name, parameter in model.named_parameters()
    ...
    groups = [{"params": decay, "weight_decay": module.WEIGHT_DECAY}]
    if candidate:
        groups.append(
            {
                "params": classifier,
                "weight_decay": module.CLASSIFIER_WEIGHT_DECAY,
            }
        )
...
def static_scope(accepted):
    ...
    new = (
        ...
        "    ]\n"
        "    classifier_params = [\n"
        "        p for name, p in model.named_parameters() if p.requires_grad and name == \"fc.weight\"\n"
        "    ]\n"
        "    no_decay_params = [\n"
        "        p for _, p in model.named_parameters() if p.requires_grad and p.ndim < 2\n"
        "    ]\n"
    )
    old_groups = '            {"params": no_decay_params, "weight_decay": 0.0},\n'
    new_groups = (
        "            {\n"
    ...
    old_constant = "WEIGHT_DECAY = 5e-4\n"
    new_constant = "WEIGHT_DECAY = 5e-4\nCLASSIFIER_WEIGHT_DECAY = 1e-3\n"
    candidate = (ROOT / "train.py").read_text()
    assert accepted.__source__.count(old) == 1 and candidate.count(new) == 1
    assert accepted.__source__.count(old_groups) == 1 and candidate.count(new_groups) == 1
    assert accepted.__source__.count(old_constant) == 1 and candidate.count(new_constant) == 1
    reconstructed = candidate.replace(new, old).replace(new_groups, old_groups).replace(new_constant, old_constant)
    assert reconstructed == accepted.__source__
...
def group_checks(accepted):
    ...
    assert sum(p.numel() for p in opt_c.param_groups[1]["params"]) == 1_280
    assert sum(p.numel() for p in opt_c.param_groups[2]["params"]) == 2_346
    assert "fc.weight" not in candidate_groups[0]
    assert candidate_groups[1] == ["fc.weight"]
    assert "fc.weight" not in candidate_groups[2]
    assert {"pooled_head.0.weight", "pooled_head.2.weight"} <= set(candidate_groups[0])
    assert [g["weight_decay"] for g in opt_c.param_groups] == [5e-4, 1e-3, 0.0]
    for key in ("lr", "momentum", "dampening", "nesterov", "maximize", "foreach"):
        assert len({g[key] for g in opt_a.param_groups + opt_c.param_groups}) == 1
...
def oracle_fixture(accepted, mixup, preseeded):
    ...
    for name in names_a:
        ...
        wd_c = 1e-3 if name == "fc.weight" else (5e-4 if pc0.ndim >= 2 else 0.0)
        if name == "fc.weight":
            torch.testing.assert_close(wd_c * pc0, 2 * wd_a * pa0, rtol=0, atol=0)
