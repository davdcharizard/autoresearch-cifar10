# Plan EXP-089: Fine Lower Flip Bracket p=0.375 Under Padding 3
- **Created**: 2026-06-10
...
- [x] Create branch `autoresearch/exp-089` from `autoresearch/dev`.
- [x] Modify only `train.py`.
- [x] Change `transforms.RandomHorizontalFlip(p=0.4)` to `transforms.RandomHorizontalFlip(p=0.375)`.
- [x] Update the startup marker to `RandomHorizontalFlip p: 0.375` so `run.log` proves EXP-089 ran the intended flip setting.
...
- [x] Confirm startup lines include selected device, `RandomCrop padding: 3 reflect`, `RandomHorizontalFlip p: 0.375`, `ResNet-20 | params: 822,790`, unchanged CutMix settings, and a 300s budget.
...
- [x] Record `best_test_acc`, `final_test_acc`, `final_test_loss`, `training_seconds`, `total_seconds`, `startup_seconds`, `peak_vram_mb`, `num_epochs`, `num_steps`, and `num_params` in `logs/exp-log-089.md`.
- [x] Classify as `improvement` only if `best_test_acc >= 94.61%`; classify smaller gains or regressions as `no-improvement`.

## Code Changes
- **train.py / transform pipeline**: Change the training transform's horizontal flip probability from 0.4 to 0.375 while keeping reflection crop padding 3.
- **train.py / startup log**: Update the flip marker to `RandomHorizontalFlip p: 0.375` so execution can verify the intended augmentation. Keep the existing `RandomCrop padding: 3 reflect` marker.

## Configuration Changes
- `RandomHorizontalFlip`: `p=0.4` -> `p=0.375`.
...
This plan intentionally differs from the failed p=0.35 experiment (EXP-083): EXP-089 starts from the stronger padding-3 anchor and uses a smaller lower-side flip move. It also avoids the newly failed p=0.425 upper-side bracket, crop padding below 3, stronger scalar decay, CutMix timing/strength, LR startup, label-smoothing, batch-size, schedule-only, policy-augmentation, cutout, and broad architecture families.
...
- Log output: Capture stdout/stderr to `run.log`; write structured observations and final metrics to `.autoresearch/logs/exp-log-089.md`.
...
- Startup markers do not confirm `RandomCrop padding: 3 reflect`, `RandomHorizontalFlip p: 0.375`, and unchanged CutMix settings.
...
3. Confirm implementation from code and log:
   ...
   - Pass condition: `RandomHorizontalFlip` uses `p=0.375`, startup log confirms `RandomCrop padding: 3 reflect` and `RandomHorizontalFlip p: 0.375`, and CutMix alpha/prob/smoothing remain 1.0/0.5/0.05.
