# Plan EXP-012: Earlier First LR Drop on Widened ResNet-20
- **Created**: 2026-06-08
...
### Milestone 1: Implement isolated scheduler retune
- [x] Change `LR_MILESTONES` in `train.py` from `[24000, 64000]` to `[22000, 64000]`.
- [x] Preserve `STAGE_WIDTHS = (20, 40, 80)`, `NUM_BLOCKS = 3`, batch size, optimizer, augmentation, seed, FP32 precision, cuDNN benchmark, channels-last, `torch.compile`, and once-per-epoch evaluation.
- [x] Run syntax/lint checks and confirm the diff only touches the planned scheduler milestone in `train.py`.
...
- [x] Monitor startup and early eval output for CUDA, compile, or throughput errors.

### Milestone 3: Capture results and classify against tightened threshold
- [x] Extract final summary metrics from `run.log`.
- [x] Compare `best_test_acc` to the current 92.12% baseline with the +0.10 point margin; EXP-012 needs `best_test_acc >= 92.22%`.
- [x] Check whether the run reaches the planned first LR drop at 22k steps and whether the extra LR 0.01 refinement improves the late plateau.
- [x] Remove temporary run logs after the result is captured in the experiment log and report.

## Code Changes
- **train.py**: change the scheduler constant from `LR_MILESTONES = [24000, 64000]` to `LR_MILESTONES = [22000, 64000]`. This isolates whether the successful widened model benefits from about 2000 additional LR 0.01 refinement steps.
- No architecture, depth, optimizer, loss, augmentation, precision, batch size, seed, dependency, or evaluation-harness changes.

## Configuration Changes
- `LR_MILESTONES`: `[24000, 64000]` -> `[22000, 64000]`.
- Rationale: EXP-011 peaked at the final epoch after entering LR 0.01 at step 24000. Moving the first milestone to 22000 tests whether more low-LR refinement can clear the new 92.22% threshold while preserving the successful width and throughput recipe. The second milestone remains unreachable to avoid the harmful LR 0.001 phase seen in EXP-003.
...
- Treat the run as no-improvement, not crash, if it completes but `best_test_acc < 92.22%`.
...
1. Confirm the baseline and threshold:
   ...
   Pass condition: output reports `baseline=92.12`; threshold is `92.22`.
...
5. Check the primary metric condition:
   parse `best_test_acc` from `run.log` and require `best_test_acc >= 92.22`.
   If the value is below `92.22`, classify EXP-012 as `no-improvement` even if it is above 92.12.
6. Schedule/throughput sanity review:
   inspect `run.log` for `lr: 0.0100`, `num_steps`, `num_epochs`, and `num_params`. The first LR drop should occur at step 22000; if the run never reaches 22k, analysis should classify the result as undertrained or schedule-missed.
7. Scope review:
   `git diff -- train.py`
   Pass condition: only the planned `LR_MILESTONES` change appears in `train.py`; `prepare.py`, dependency files, and evaluation harness are unchanged.
