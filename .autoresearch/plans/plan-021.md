# Plan EXP-021: Sparse Post-Drop Weight Averaging
- **Created**: 2026-06-08
...
### Milestone 1: Implement sparse post-drop averaging
- [x] Create experiment branch `autoresearch/exp-021` from `autoresearch/dev`.
- [x] Modify only `train.py`.
- [x] Add `AveragedModel` support for a sparse averaged model.
- [x] Preserve `STAGE_WIDTHS = (28, 56, 112)` and `LR_MILESTONES = [21000, 64000]`.
- [x] Keep training on the compiled model, but update the averaged model from the underlying uncompiled model once per epoch after the first LR drop.
- [x] Evaluate exactly one model per epoch: live model before averaging starts, averaged model after averaging starts.
- [x] Print enough evaluation context to confirm whether the live or averaged model was evaluated and how many averaged snapshots have been accumulated.
- [x] Run `python3 -m py_compile train.py`.
- [x] Run `uv run ruff check train.py`.
- [x] Confirm `git diff -- train.py` is limited to the planned averaging changes.
...
- [x] Confirm post-drop log lines show averaged-model evaluation after the 21k step threshold is reached.
...
- [x] Compare `best_test_acc` to the concrete EXP-021 threshold `93.33%` (`93.23 + 0.10`).
- [x] Record verdict and metrics in `.autoresearch/logs/exp-log-021.md`; the experiment index is updated during analysis.
- [ ] On improvement, commit `train.py`, merge into `autoresearch/dev`, and skip PR creation if no remote exists.
- [x] On no-improvement, crash, or invalid result, revert `train.py` averaging changes with `apply_patch` and return to `autoresearch/dev`.
- [x] Remove `run.log` and root `__pycache__` after metrics are captured.

## Code Changes
- **train.py**: Import `AveragedModel` from `torch.optim.swa_utils`.
- **train.py**: Add constants for sparse averaging, including an enable flag and `AVG_START_STEP = 21000`.
- **train.py**: Keep a raw `ResNet` module reference before optional `torch.compile`, compile only the training wrapper, and create `AveragedModel(raw_model, device=device, use_buffers=True)`.
- **train.py**: After each epoch, if `step >= AVG_START_STEP`, call `avg_model.update_parameters(raw_model)` once and evaluate `avg_model`; otherwise evaluate the live compiled training model.
- **train.py**: Preserve one call site to `evaluator.evaluate(...)` per epoch by selecting `eval_model` conditionally before the existing evaluation call.
- **train.py**: Include the evaluation source (`live` or `avg`) and averaged snapshot count in the epoch evaluation print line so the run log proves the planned path was exercised.

## Configuration Changes
- `STAGE_WIDTHS`: unchanged at `(28, 56, 112)` to preserve the current best architecture.
- `LR_MILESTONES`: unchanged at `[21000, 64000]` to preserve the current best schedule.
- `AVG_START_STEP`: new constant set to `21000`, matching the first LR drop so averaging only covers low-LR refinement.
- `AVG_USE_BUFFERS`: new constant initially planned as `True`, revised to `False` after Run 1 because this PyTorch version cannot average integer BatchNorm bookkeeping buffers with the default SWA update; `AveragedModel(use_buffers=False)` keeps buffers synchronized by copying them from the source model.
- All other settings are unchanged: `NUM_BLOCKS`, `BATCH_SIZE`, `LR`, `MOMENTUM`, `WEIGHT_DECAY`, `MAX_STEPS`, `USE_CUDNN_BENCHMARK`, `USE_CHANNELS_LAST`, `USE_COMPILE`, seed, augmentation, optimizer, and once-per-epoch validation.
...
- Log output: stdout and stderr captured in project-root `run.log`, then summarized into `.autoresearch/logs/exp-log-021.md` before cleanup.
...
- Treat the result as no-improvement if averaged-model evaluation never activates after the 21k first LR drop, because the planned intervention was not actually tested.
- Treat the result as no-improvement if sparse averaging reduces the step budget so much that the first LR drop is not reached.
...
1. Verify baseline and threshold:
   ...
   - Pass condition: output reports `baseline=93.23`; the EXP-021 improvement threshold is `best_test_acc >= 93.33`.
   - Timeout: 30 seconds.
...
4. Verify validation cadence:
   ...
   - Timeout: 30 seconds.
5. Verify sparse averaging activation:
   - Command: `grep "eval avg" run.log`
   - Pass condition: at least one epoch evaluation line is labeled as averaged-model evaluation after the first LR drop.
   - Timeout: 30 seconds.
...
7. Verify metric improvement:
   ...
   - Timeout: 30 seconds.
8. Verify schedule and hard constraints:
   ...
   - Pass condition: the 21k first drop was reached, only planned `train.py` changes were present during the run, fixed training budget was used, total wall-clock stayed under 10 minutes, and no protected files changed.
   - Timeout: 1 minute.
