# Plan EXP-022: 20k First LR Drop on 28/56/112
- **Created**: 2026-06-08
...
### Milestone 1: Implement isolated schedule change
- [x] Create experiment branch `autoresearch/exp-022` from `autoresearch/dev`.
- [x] Modify only `train.py`.
- [x] Change `LR_MILESTONES` from `[21000, 64000]` to `[20000, 64000]`.
- [x] Preserve `STAGE_WIDTHS = (28, 56, 112)`, `WEIGHT_DECAY = 1e-4`, `BATCH_SIZE = 128`, FP32 training, channels-last, cuDNN benchmark, and `torch.compile`.
- [x] Run `python3 -m py_compile train.py`.
- [x] Run `uv run ruff check train.py`.
- [x] Confirm `git diff -- train.py` contains only the planned LR milestone change.
...
- [x] Confirm the first LR drop occurs at step 20000 by checking `run.log` for `step 20000` and `lr: 0.0100`.
...
- [x] Compare `best_test_acc` to the concrete EXP-022 threshold `93.33%` (`93.23 + 0.10`).
- [x] Record verdict and metrics in `.autoresearch/logs/exp-log-022.md`; the experiment index is updated during analysis.
...
## Code Changes
- **train.py**: Change the first LR milestone from 21000 to 20000 while keeping the second milestone at 64000. This isolates whether the current-best 28/56/112 anchor benefits from one adjacent step earlier LR 0.01 refinement.

## Configuration Changes
- `LR_MILESTONES`: `[21000, 64000]` -> `[20000, 64000]` to test the next earlier first-drop bracket.
- `STAGE_WIDTHS`: unchanged at `(28, 56, 112)`.
- `WEIGHT_DECAY`: unchanged at `1e-4`.
- `BATCH_SIZE`, `LR`, `MOMENTUM`, `MAX_STEPS`, `USE_CUDNN_BENCHMARK`, `USE_CHANNELS_LAST`, `USE_COMPILE`, seed, augmentation, optimizer, and once-per-epoch validation remain unchanged.
...
- Log output: stdout and stderr captured in project-root `run.log`, then summarized into `.autoresearch/logs/exp-log-022.md` before cleanup.
...
- Treat the result as no-improvement if the first LR drop does not occur at step 20000, because the planned intervention was not actually tested.
- Treat the result as no-improvement if the schedule change reduces useful training progress so much that post-drop accuracy never reaches the current anchor's neighborhood.
...
1. Verify baseline and threshold:
   ...
   - Pass condition: output reports `baseline=93.23`; the EXP-022 improvement threshold is `best_test_acc >= 93.33`.
   - Timeout: 30 seconds.
...
4. Verify validation cadence:
   ...
   - Timeout: 30 seconds.
5. Verify schedule activation:
   - Command: `grep "step 20000" run.log` and inspect that the same progress line includes `lr: 0.0100`.
   - Pass condition: first LR drop is reached at step 20000 during the run.
   - Timeout: 30 seconds.
