# Plan EXP-033: Lower Label Smoothing to 0.03
- **Created**: 2026-06-08
...
### Milestone 1: Implement lower label smoothing value
- [x] Create experiment branch `autoresearch/exp-033` from `autoresearch/dev`.
...
- [x] Confirm `git diff -- train.py` contains only the planned label-smoothing scalar change.
- [x] Confirm the new loss call is present with `rg -n "F\\.cross_entropy\\(outputs, targets, label_smoothing=0\\.03\\)" train.py`.
...
- [x] Compare `best_test_acc` to the concrete EXP-033 threshold `93.80%` (`93.70 + 0.10`).
- [x] Record verdict and metrics in `.autoresearch/logs/exp-log-033.md`; the experiment index is updated during analysis.
- [ ] On improvement, commit `train.py`, merge into `autoresearch/dev`, and skip PR creation if no remote exists.
- [x] On no-improvement, crash, or invalid result, revert the `label_smoothing=0.03` loss-call change with `apply_patch` and return to `autoresearch/dev`.
...
This tests whether the newly validated label-smoothing anchor is better with slightly less confidence regularization.
...
- Log output: stdout and stderr captured in project-root `run.log`, then summarized into `.autoresearch/logs/exp-log-033.md` before cleanup.
...
1. Verify baseline and threshold:
   ...
   - Pass condition: output reports `baseline=93.70`; the EXP-033 improvement threshold is `best_test_acc >= 93.80`.
   - Timeout: 30 seconds.
...
4. Verify validation cadence, loss change, augmentation, and preserved schedule:
   - Commands: `rg -n "evaluator\\.evaluate|Eval\\(" train.py`, `rg -n "F\\.cross_entropy\\(outputs, targets, label_smoothing=0\\.03\\)" train.py`, `rg -n "RandomCrop\\(32, padding=4, padding_mode=\"reflect\"\\)" train.py`, and `rg -n "LR_MILESTONES = \\[21000, 64000\\]" train.py`
   - Pass condition: one `Eval()` construction, one epoch-level `evaluator.evaluate(...)` call, the 0.03 label-smoothing loss call is present, reflected crop padding is preserved, and the anchor schedule is preserved.
   - Timeout: 30 seconds.
...
8. Verify hard constraints:
   ...
   - Pass condition: only the planned label-smoothing scalar diff was present during the run, fixed training budget was used, total wall-clock stayed under 10 minutes, and no protected files changed.
   - Timeout: 1 minute.
