# Plan EXP-016: 28/56/112 First LR Drop at 21k
- **Created**: 2026-06-08
...
### Milestone 1: Implement isolated earlier schedule retune
- [x] Change `LR_MILESTONES` in `train.py` from `[22000, 64000]` to `[21000, 64000]`.
- [x] Preserve `STAGE_WIDTHS = (28, 56, 112)`, `NUM_BLOCKS = 3`, batch size, optimizer, augmentation, seed, FP32 precision, cuDNN benchmark, channels-last, `torch.compile`, and once-per-epoch evaluation.
- [x] Run syntax/lint checks and confirm the diff only touches the planned scheduler constant in `train.py`.
...
- [x] Compare `best_test_acc` to the current 93.09% baseline with the +0.10 point margin; EXP-016 needs `best_test_acc >= 93.19%`.
- [x] Check whether the run reaches the planned first LR drop at 21k steps and retains enough LR 0.01 refinement time.
- [x] Remove temporary run logs after the result is captured in the experiment log and report.

## Code Changes
- **train.py**: change `LR_MILESTONES = [22000, 64000]` to `LR_MILESTONES = [21000, 64000]`.
- No architecture, depth, optimizer, loss, augmentation, precision, batch size, seed, dependency, or evaluation-harness changes.

## Configuration Changes
- `LR_MILESTONES`: `[22000, 64000]` -> `[21000, 64000]`. Rationale: EXP-015 showed 23k is too late for 28/56/112, while EXP-014's 22k drop is current best. A 21k drop tests whether slightly more LR 0.01 refinement can improve the same architecture without confounding capacity or throughput settings.
- `STAGE_WIDTHS`: unchanged at `(28, 56, 112)` to isolate the schedule effect.
...
- Treat the run as no-improvement, not crash, if it completes but `best_test_acc < 93.19%`.
...
1. Confirm the baseline and threshold:
   ...
   Pass condition: output reports `baseline=93.09`; threshold is `93.19`.
...
5. Check the primary metric condition:
   parse `best_test_acc` from `run.log` and require `best_test_acc >= 93.19`.
   If the value is below `93.19`, classify EXP-016 as `no-improvement` even if it is above 93.09.
6. Schedule/throughput sanity review:
   inspect `run.log` for `lr: 0.0100`, `num_steps`, `num_epochs`, and `num_params`. The run should reach the first LR drop at step 21000; if it never reaches 21k, analysis should classify the result as undertrained or schedule-missed.
7. Scope review:
   `git diff -- train.py`
