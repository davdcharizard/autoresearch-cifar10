# Plan EXP-084: Horizontal Flip Probability 0.45
- **Created**: 2026-06-10
...
- [x] Create branch `autoresearch/exp-084` from `autoresearch/dev`.
- [x] Modify only `train.py`.
- [x] Change `transforms.RandomHorizontalFlip(p=0.4)` to `transforms.RandomHorizontalFlip(p=0.45)`.
...
- [x] Update the startup marker to `RandomHorizontalFlip p: 0.45` so `run.log` proves EXP-084 ran the intended transform.
...
- [x] Record `best_test_acc`, `final_test_acc`, `final_test_loss`, `training_seconds`, `total_seconds`, `startup_seconds`, `peak_vram_mb`, `num_epochs`, `num_steps`, and `num_params` in `logs/exp-log-084.md`.
- [x] Classify as `improvement` only if `best_test_acc >= 94.46%`; classify smaller gains or regressions as `no-improvement`.

## Code Changes
- **train.py / transform pipeline**: Change the training transform's horizontal flip probability from `p=0.4` to `p=0.45`.
- **train.py / startup log**: Update the marker from `RandomHorizontalFlip p: 0.4` to `RandomHorizontalFlip p: 0.45` so execution can verify the intended augmentation.

## Configuration Changes
- `RandomHorizontalFlip`: `p=0.4` -> `p=0.45`. This completes the upper side of the local flip-probability bracket after EXP-082 validated `p=0.4` and EXP-083 showed `p=0.35` regresses.
...
This plan intentionally differs from failed augmentation families. It does not add policy augmentation, photometric jitter, or erased-patch masking; it only adjusts one existing no-overhead transform. It also avoids closed CutMix timing/strength, LR startup, label-smoothing, batch-size, schedule-only, and architecture families.
...
- Log output: Capture stdout/stderr to `run.log`; write structured observations and final metrics to `.autoresearch/logs/exp-log-084.md`.
...
- Startup markers do not confirm unchanged CutMix settings and `RandomHorizontalFlip p: 0.45`.
...
3. Confirm implementation from code and log:
   ...
   - Pass condition: `RandomHorizontalFlip` uses `p=0.45`, startup log confirms `RandomHorizontalFlip p: 0.45`, and CutMix alpha/prob/smoothing remain 1.0/0.5/0.05.
