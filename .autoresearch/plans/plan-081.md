# Plan EXP-081: Reflection Crop Padding 3
- **Created**: 2026-06-09
...
- [x] Create branch `autoresearch/exp-081` from `autoresearch/dev`.
- [x] Modify only `train.py`.
- [x] Change `transforms.RandomCrop(32, padding=4, padding_mode="reflect")` to `transforms.RandomCrop(32, padding=3, padding_mode="reflect")`.
- [x] Keep `padding_mode="reflect"`, `RandomHorizontalFlip()`, unit-std normalization, CutMix alpha/probability/label smoothing, clean label smoothing, architecture, optimizer, LR milestones, batch size, seed, compile/channels-last, and validation cadence unchanged.
- [x] Add a startup marker such as `RandomCrop padding: 3 reflect` so `run.log` proves EXP-081 ran the intended transform.
...
- [x] Confirm startup lines include selected device, `ResNet-20 | params: 822,790`, unchanged CutMix settings, and the RandomCrop padding marker.
...
- [x] Record `best_test_acc`, `final_test_acc`, `final_test_loss`, `training_seconds`, `total_seconds`, `startup_seconds`, `peak_vram_mb`, `num_epochs`, `num_steps`, and `num_params` in `logs/exp-log-081.md`.
- [x] Classify as `improvement` only if `best_test_acc >= 94.21%`; classify smaller gains or regressions as `no-improvement`.

## Code Changes
- **train.py / transform pipeline**: Change the training transform's `RandomCrop` padding from 4 to 3 while preserving `padding_mode="reflect"`.
- **train.py / startup log**: Print `RandomCrop padding: 3 reflect` after transform setup or before training starts so execution can verify the intended augmentation.

## Configuration Changes
- `RandomCrop` padding: `4 -> 3`. This slightly weakens spatial jitter while keeping reflection boundary behavior, testing whether the CutMix anchor is marginally over-regularized.
- All other configuration remains unchanged: `CUTMIX_ALPHA=1.0`, `CUTMIX_PROB=0.5`, `CUTMIX_LABEL_SMOOTHING=0.05`, clean-batch label smoothing 0.05, `STAGE_WIDTHS=(28, 56, 112)`, `WEIGHT_DECAY=2e-4`, `LR=0.1`, `LR_MILESTONES=[21000, 64000]`, batch size 128, FP32 compile/channels-last, and seed 42.

This plan intentionally differs from failed augmentation approaches. It does not add policy augmentation, ColorJitter, or Cutout; it preserves the validated reflection padding mode and only brackets padding magnitude. It also avoids closed CutMix timing/strength, LR startup, label-smoothing, and architecture families.
...
- Log output: Capture stdout/stderr to `run.log`; write structured observations and final metrics to `.autoresearch/logs/exp-log-081.md`.
...
- Startup markers do not confirm unchanged CutMix settings and `RandomCrop padding: 3 reflect`.
...
3. Confirm implementation from code and log:
   - Command: inspect `git diff train.py` and `grep "CutMix alpha:\|RandomCrop padding:" run.log`.
   - Pass condition: `RandomCrop` uses `padding=3`, startup log confirms `RandomCrop padding: 3 reflect`, and CutMix alpha/prob/smoothing remain 1.0/0.5/0.05.
