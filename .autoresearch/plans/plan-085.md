# Plan EXP-085: Crop Padding 3 on Flip p=0.4 Anchor
- **Created**: 2026-06-10
...
- [x] Create branch `autoresearch/exp-085` from `autoresearch/dev`.
- [x] Modify only `train.py`.
- [x] Change `transforms.RandomCrop(32, padding=4, padding_mode="reflect")` to `padding=3`.
...
- [x] Add or update a startup marker to `RandomCrop padding: 3 reflect` so `run.log` proves EXP-085 ran the intended crop setting.
...
- [x] Confirm startup lines include selected device, `RandomCrop padding: 3 reflect`, `RandomHorizontalFlip p: 0.4`, `ResNet-20 | params: 822,790`, and unchanged CutMix settings.
...
- [x] Record `best_test_acc`, `final_test_acc`, `final_test_loss`, `training_seconds`, `total_seconds`, `startup_seconds`, `peak_vram_mb`, `num_epochs`, `num_steps`, and `num_params` in `logs/exp-log-085.md`.
- [x] Classify as `improvement` only if `best_test_acc >= 94.46%`; classify smaller gains or regressions as `no-improvement`.

## Code Changes
- **train.py / transform pipeline**: Change the training transform's reflection crop padding from 4 to 3 while keeping `RandomHorizontalFlip(p=0.4)`.
- **train.py / startup log**: Add or update the crop marker to `RandomCrop padding: 3 reflect` so execution can verify the intended crop augmentation. Keep the existing `RandomHorizontalFlip p: 0.4` marker.

## Configuration Changes
- `RandomCrop`: `padding=4` -> `padding=3` with `padding_mode="reflect"`. This tests whether the prior padding-3 near miss becomes useful on the validated `RandomHorizontalFlip(p=0.4)` anchor.
...
This plan intentionally differs from the closed flip-probability bracket. It preserves the proven p=0.4 flip setting and changes only crop jitter strength. It also avoids closed CutMix timing/strength, LR startup, label-smoothing, batch-size, schedule-only, policy-augmentation, and architecture families.
...
- Log output: Capture stdout/stderr to `run.log`; write structured observations and final metrics to `.autoresearch/logs/exp-log-085.md`.
...
3. Confirm implementation from code and log:
   ...
   - Pass condition: `RandomCrop` uses `padding=3`, startup log confirms `RandomCrop padding: 3 reflect` and `RandomHorizontalFlip p: 0.4`, and CutMix alpha/prob/smoothing remain 1.0/0.5/0.05.
