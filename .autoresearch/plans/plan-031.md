# Plan EXP-031: Symmetric Padding for RandomCrop
- **Created**: 2026-06-08
...
### Milestone 1: Implement symmetric crop padding
- [x] Create experiment branch `autoresearch/exp-031` from `autoresearch/dev`.
- [x] Modify only `train.py`.
- [x] Change `transforms.RandomCrop(32, padding=4, padding_mode="reflect")` to `transforms.RandomCrop(32, padding=4, padding_mode="symmetric")`.
...
- [x] Confirm torchvision accepts the symmetric padding mode with `uv run python -c 'from torchvision import transforms; transforms.RandomCrop(32, padding=4, padding_mode="symmetric")'`.
- [x] Confirm validation cadence remains once per epoch with `rg -n "evaluator\\.evaluate|Eval\\(" train.py`.
- [x] Confirm the new padding mode is present with `rg -n "RandomCrop\\(32, padding=4, padding_mode=\"symmetric\"\\)" train.py`.
...
- [x] Compare `best_test_acc` to the concrete EXP-031 threshold `93.68%` (`93.58 + 0.10`).
- [x] Record verdict and metrics in `.autoresearch/logs/exp-log-031.md`; the experiment index is updated during analysis.
- [x] On improvement, commit `train.py`, merge into `autoresearch/dev`, and skip PR creation if no remote exists. Not applicable for EXP-031 because the verdict was no-improvement.
- [x] On no-improvement, crash, or invalid result, revert the `RandomCrop` padding-mode change with `apply_patch` and return to `autoresearch/dev`.
- [x] Remove `run.log` and root `__pycache__` after metrics are captured and analyzed.

## Code Changes
- **train.py**:
  - Change CIFAR training augmentation from reflected crop padding to symmetric crop padding:
    `transforms.RandomCrop(32, padding=4, padding_mode="symmetric")`.
  - Leave all architecture, optimizer, batch-size, compile, seed, schedule, and validation settings unchanged.

This tests whether the no-zero-border crop-boundary mechanism from EXP-029 is better served by symmetric edge mirroring than by reflection.

## Configuration Changes
- `RandomCrop.padding_mode`: `"reflect"` -> `"symmetric"`.
- No model, optimizer, weight-decay, batch-size, LR schedule, seed, dependency, or harness configuration changes.
...
- Log output: stdout and stderr captured in project-root `run.log`, then summarized into `.autoresearch/logs/exp-log-031.md` before cleanup.
...
- Treat the result as invalid if `num_params` differs from `822,790`, because the experiment is intended to change augmentation boundary fill only.
...
1. Verify baseline and threshold:
   ...
   - Pass condition: output reports `baseline=93.58`; the EXP-031 improvement threshold is `best_test_acc >= 93.68`.
   - Timeout: 30 seconds.
2. Verify scope before launch:
   ...
   - Timeout: 30 seconds.
3. Verify syntax, lint, and transform validity:
   - Commands: `python3 -m py_compile train.py`, `uv run ruff check train.py`, and `uv run python -c 'from torchvision import transforms; transforms.RandomCrop(32, padding=4, padding_mode="symmetric")'`
   - Pass condition: all commands exit 0.
   - Timeout: 2 minutes total.
4. Verify validation cadence, augmentation, and preserved schedule:
   - Commands: `rg -n "evaluator\\.evaluate|Eval\\(" train.py`, `rg -n "RandomCrop\\(32, padding=4, padding_mode=\"symmetric\"\\)" train.py`, and `rg -n "LR_MILESTONES = \\[21000, 64000\\]" train.py`
   - Pass condition: one `Eval()` construction, one epoch-level `evaluator.evaluate(...)` call, symmetric crop padding is present, and the anchor schedule is preserved.
   - Timeout: 30 seconds.
...
8. Verify hard constraints:
   ...
   - Pass condition: only the planned padding-mode diff was present during the run, fixed training budget was used, total wall-clock stayed under 10 minutes, and no protected files changed.
   - Timeout: 1 minute.
