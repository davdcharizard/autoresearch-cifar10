# Plan EXP-034: Move First LR Drop to 22k on Label-Smoothed Anchor
- **Created**: 2026-06-08
...
### Milestone 1: Implement 22k first LR drop
- [x] Create experiment branch `autoresearch/exp-034` from `autoresearch/dev`.
- [x] Modify only `train.py`.
- [x] Change `LR_MILESTONES = [21000, 64000]` to `LR_MILESTONES = [22000, 64000]`.
- [x] Preserve `STAGE_WIDTHS = (28, 56, 112)`, reflected `RandomCrop`, `label_smoothing=0.05`, `BATCH_SIZE = 128`, `LR = 0.1`, `MOMENTUM = 0.9`, `WEIGHT_DECAY = 1e-4`, FP32 training, channels-last, cuDNN benchmark, `torch.compile`, seed, optimizer class, and once-per-epoch validation.
- [x] Run `python3 -m py_compile train.py`.
- [x] Run `uv run ruff check train.py`.
- [x] Confirm `git diff -- train.py` contains only the planned LR milestone scalar change.
- [x] Confirm the new schedule is present with `rg -n "LR_MILESTONES = \\[22000, 64000\\]" train.py`.
- [x] Confirm the label-smoothed anchor is preserved with `rg -n "label_smoothing=0\\.05" train.py`.
...
- [x] Compare `best_test_acc` to the concrete EXP-034 threshold `93.80%` (`93.70 + 0.10`).
- [x] Record verdict and metrics in `.autoresearch/logs/exp-log-034.md`; the experiment index is updated during analysis.
- [ ] On improvement, commit `train.py`, merge into `autoresearch/dev`, and skip PR creation if no remote exists.
- [x] On no-improvement, crash, or invalid result, revert the `[22000, 64000]` schedule change with `apply_patch` and return to `autoresearch/dev`.
...
- **train.py**:
  ...
  - Leave `label_smoothing=0.05` and the evaluation path untouched.
  - Leave all architecture, augmentation, optimizer, batch-size, compile, seed, and validation settings unchanged.

This tests whether the newly label-smoothed anchor benefits from slightly more LR 0.1 fitting before LR 0.01 refinement.

## Configuration Changes
- `LR_MILESTONES`: `[21000, 64000]` -> `[22000, 64000]`.
- No model, augmentation, optimizer, weight-decay, batch-size, label-smoothing, seed, dependency, or harness configuration changes.
...
- Log output: stdout and stderr captured in project-root `run.log`, then summarized into `.autoresearch/logs/exp-log-034.md` before cleanup.
...
- Treat the result as invalid if `num_params` differs from `822,790`, because the experiment is intended to change schedule timing only.
...
1. Verify baseline and threshold:
   ...
   - Pass condition: output reports `baseline=93.70`; the EXP-034 improvement threshold is `best_test_acc >= 93.80`.
   - Timeout: 30 seconds.
...
4. Verify validation cadence, schedule change, augmentation, and preserved smoothing:
   - Commands: `rg -n "evaluator\\.evaluate|Eval\\(" train.py`, `rg -n "LR_MILESTONES = \\[22000, 64000\\]" train.py`, `rg -n "label_smoothing=0\\.05" train.py`, and `rg -n "RandomCrop\\(32, padding=4, padding_mode=\"reflect\"\\)" train.py`
   - Pass condition: one `Eval()` construction, one epoch-level `evaluator.evaluate(...)` call, the 22k schedule is present, 0.05 label smoothing is preserved, and reflected crop padding is preserved.
   - Timeout: 30 seconds.
...
8. Verify hard constraints:
   ...
   - Pass condition: only the planned LR milestone diff was present during the run, fixed training budget was used, total wall-clock stayed under 10 minutes, and no protected files changed.
   - Timeout: 1 minute.
