# Plan EXP-026: Momentum 0.95 on Current Anchor
- **Created**: 2026-06-08
...
### Milestone 1: Implement isolated momentum change
- [x] Create experiment branch `autoresearch/exp-026` from `autoresearch/dev`.
- [x] Modify only `train.py`.
- [x] Change `MOMENTUM` from `0.9` to `0.95`.
- [x] Preserve `STAGE_WIDTHS = (28, 56, 112)`, `BATCH_SIZE = 128`, `LR = 0.1`, `WEIGHT_DECAY = 1e-4`, `LR_MILESTONES = [21000, 64000]`, FP32 training, channels-last, cuDNN benchmark, `torch.compile`, seed, augmentation, optimizer class, and once-per-epoch validation.
- [x] Run `python3 -m py_compile train.py`.
- [x] Run `uv run ruff check train.py`.
- [x] Confirm `git diff -- train.py` contains only the planned `MOMENTUM` change.
...
- [x] Compare `best_test_acc` to the concrete EXP-026 threshold `93.33%` (`93.23 + 0.10`).
- [x] Record verdict and metrics in `.autoresearch/logs/exp-log-026.md`; the experiment index is updated during analysis.
- [ ] On improvement, commit `train.py`, merge into `autoresearch/dev`, and skip PR creation if no remote exists.
- [x] On no-improvement, crash, or invalid result, revert `train.py` momentum change with `apply_patch` and return to `autoresearch/dev`.
- [x] Remove `run.log` and root `__pycache__` after metrics are captured.

## Code Changes
- **train.py**:
  - Change `MOMENTUM` from `0.9` to `0.95`.

This tests whether a higher classical SGD momentum coefficient improves post-drop refinement on the validated 28/56/112 anchor without reducing throughput, changing capacity, or adding explicit regularization.

## Configuration Changes
- `MOMENTUM`: `0.9` -> `0.95`.
- `STAGE_WIDTHS`: unchanged at `(28, 56, 112)`.
- `BATCH_SIZE`: unchanged at `128`.
- `LR`: unchanged at `0.1`.
- `WEIGHT_DECAY`: unchanged at `1e-4`.
- `LR_MILESTONES`: unchanged at `[21000, 64000]`.
- `MAX_STEPS`, `USE_CUDNN_BENCHMARK`, `USE_CHANNELS_LAST`, `USE_COMPILE`, seed, augmentation, optimizer class, and once-per-epoch validation remain unchanged.
...
- Log output: stdout and stderr captured in project-root `run.log`, then summarized into `.autoresearch/logs/exp-log-026.md` before cleanup.
...
1. Verify baseline and threshold:
   ...
   - Pass condition: output reports `baseline=93.23`; the EXP-026 improvement threshold is `best_test_acc >= 93.33`.
   - Timeout: 30 seconds.
...
4. Verify validation cadence:
   ...
   - Timeout: 30 seconds.
5. Verify preserved batch size and schedule:
...
8. Verify hard constraints:
   ...
   - Pass condition: only the planned `MOMENTUM = 0.95` diff was present during the run, fixed training budget was used, total wall-clock stayed under 10 minutes, and no protected files changed.
   - Timeout: 1 minute.
