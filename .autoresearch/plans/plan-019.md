# Plan EXP-019: Minimal Width Step 29/58/116 with 19k First LR Drop
- **Created**: 2026-06-08
...
### Milestone 1: Implement width/schedule constants and validate scope
- [x] Create experiment branch `autoresearch/exp-019` from `autoresearch/dev`.
- [x] Modify only `train.py`, changing `STAGE_WIDTHS` from `(28, 56, 112)` to `(29, 58, 116)`.
- [x] Modify only `train.py`, changing `LR_MILESTONES` from `[21000, 64000]` to `[19000, 64000]`.
...
- [x] Confirm CUDA sees one selected GPU with `CUDA_VISIBLE_DEVICES=1 uv run python -c 'import torch; print(torch.cuda.is_available()); print(torch.cuda.device_count()); print(torch.cuda.get_device_name(0) if torch.cuda.is_available() else "no cuda")'`.
- [x] Remove stale `run.log` before launch.
- [x] Launch `CUDA_VISIBLE_DEVICES=1 uv run train.py > run.log 2>&1`.
...
- [x] Compare `best_test_acc` to the concrete EXP-019 threshold `93.33%` (`93.23 + 0.10`).
- [x] Record verdict and metrics in `.autoresearch/logs/exp-log-019.md` and the experiment index.
- [ ] On improvement, commit `train.py`, merge into `autoresearch/dev`, and skip PR creation if no remote exists.
- [x] On no-improvement, crash, or invalid result, revert the `train.py` constant changes with `apply_patch` and return to `autoresearch/dev`.
- [x] Remove `run.log` and root `__pycache__` after metrics are captured.

## Code Changes
- **train.py**: Change `STAGE_WIDTHS = (28, 56, 112)` to `STAGE_WIDTHS = (29, 58, 116)`. This adds a smaller proportional width increment than EXP-017's failed `(30, 60, 120)` step while preserving the existing ResNet-20 architecture.
- **train.py**: Change `LR_MILESTONES = [21000, 64000]` to `LR_MILESTONES = [19000, 64000]`. This gives the slightly wider model more LR 0.01 refinement time under the fixed step budget and directly responds to EXP-017's limited post-drop window.

## Configuration Changes
- `STAGE_WIDTHS`: `(28, 56, 112)` -> `(29, 58, 116)` to test a minimal next width step.
- `LR_MILESTONES`: `[21000, 64000]` -> `[19000, 64000]` to compensate for expected throughput loss and preserve post-drop refinement.
...
- Method: local command from the project root, preferring an idle single GPU; actual launch command is `CUDA_VISIBLE_DEVICES=1 uv run train.py > run.log 2>&1` because physical GPU 0 was busy and GPU 1 was idle.
...
- Log output: stdout and stderr captured in project-root `run.log`, then summarized into `.autoresearch/logs/exp-log-019.md` before cleanup.
...
- Treat the result as no-improvement if the first LR drop at step 19000 is not reached before the run ends, because the planned schedule was not actually tested.
...
1. Verify baseline and threshold:
   ...
   - Pass condition: output reports `baseline=93.23`; the EXP-019 improvement threshold is `best_test_acc >= 93.33`.
   - Timeout: 30 seconds.
...
7. Verify schedule and hard constraints:
   ...
   - Pass condition: the 19k first drop was reached, only the planned constants changed during the run, fixed training budget was used, total wall-clock stayed under 10 minutes, and no protected files changed.
   - Timeout: 1 minute.
...
- num_params: `grep "^num_params:" run.log` — confirms the parameter cost of the smaller width step.
