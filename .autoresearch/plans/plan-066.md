# Plan EXP-066: CutMix Probability 0.75
- **Created**: 2026-06-09
...
- [x] Create experiment branch `autoresearch/exp-066` from `autoresearch/dev`.
- [x] Change only `train.py`, setting `CUTMIX_PROB = 0.75`.
- [x] Confirm `git diff --name-only` lists only `train.py`.
- [x] Run `python3 -m py_compile train.py`.
- [x] Run `uv run ruff check train.py` if ruff is available through the project environment.
...
- [x] Confirm startup markers in `run.log`: CUDA device, ResNet-20 parameter count, `CutMix alpha: 1.0, prob: 0.75, label smoothing: 0.05`, 300s budget, and 390 batches per epoch.
...
- [x] Update `logs/exp-log-066.md` with final metrics and verification results.

## Code Changes
- **train.py**: Change `CUTMIX_PROB` from `0.5` to `0.75`. This tests stronger regional CutMix exposure while preserving the validated EXP-064 implementation, alpha, smoothing, architecture, optimizer, LR schedule, augmentation, compile path, and validation cadence.

## Configuration Changes
- `CUTMIX_PROB`: `0.5` -> `0.75` (completes the high-frequency bracket after EXP-065 showed `0.25` slightly underperformed the new CutMix anchor).

## Execution Environment
- Method: local single-GPU foreground command from the project root.
- Resources: one available NVIDIA H20 class GPU; expected VRAM similar to EXP-064/065, around 660 MB peak.
...
- Abort if startup markers do not show `CUTMIX_PROB=0.75` or if the run is not using CUDA.
...
1. Confirm branch and diff:
   ...
   - Pass condition: branch is `autoresearch/exp-066`; tracked diff is limited to `train.py`; the only intended code change is `CUTMIX_PROB = 0.75`.
   - Timeout: 30 seconds.
2. Confirm syntax and lint:
   ...
   - Pass condition: exits with status 0, unless ruff is unavailable in the project environment; if unavailable, record that in the exp-log and continue because this project has previously used it as a best-effort check.
   - Timeout: 60 seconds.
...
4. Verify startup and scope markers:
   ...
   - Pass condition: CUDA device is used, ResNet-20 has 822,790 params, CutMix line reports `alpha: 1.0, prob: 0.75, label smoothing: 0.05`, time budget is 300s, and batches per epoch is 390.
   - Timeout: 30 seconds.
...
7. Compare against current baseline:
   - Baseline from experiment index before EXP-066: `94.11%`; required improvement threshold: `94.21%`.
