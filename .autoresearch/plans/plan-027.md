# Plan EXP-027: Exclude BatchNorm and Bias from Weight Decay
- **Created**: 2026-06-08
...
### Milestone 1: Implement optimizer parameter groups
- [x] Create experiment branch `autoresearch/exp-027` from `autoresearch/dev`.
- [x] Modify only `train.py`.
- [x] Replace the single `model.parameters()` SGD parameter set with two parameter groups:
  - decay group: parameters with `param.ndim > 1` and names not ending in `.bias`, using `weight_decay=WEIGHT_DECAY`.
  - no-decay group: 1D parameters and bias parameters, using `weight_decay=0.0`.
- [x] Preserve `STAGE_WIDTHS = (28, 56, 112)`, `BATCH_SIZE = 128`, `LR = 0.1`, `MOMENTUM = 0.9`, `WEIGHT_DECAY = 1e-4`, `LR_MILESTONES = [21000, 64000]`, FP32 training, channels-last, cuDNN benchmark, `torch.compile`, seed, augmentation, optimizer class, and once-per-epoch validation.
- [x] Run `python3 -m py_compile train.py`.
- [x] Run `uv run ruff check train.py`.
- [x] Confirm `git diff -- train.py` contains only the planned optimizer param-group change.
- [x] Confirm validation cadence remains once per epoch with `rg -n "evaluator\\.evaluate|Eval\\(" train.py`.
- [x] Confirm the optimizer setup contains both `weight_decay=WEIGHT_DECAY` and `weight_decay=0.0` with `rg -n "decay_params|no_decay_params|weight_decay" train.py`.
...
- [x] Compare `best_test_acc` to the concrete EXP-027 threshold `93.33%` (`93.23 + 0.10`).
- [x] Record verdict and metrics in `.autoresearch/logs/exp-log-027.md`; the experiment index is updated during analysis.
- [ ] On improvement, commit `train.py`, merge into `autoresearch/dev`, and skip PR creation if no remote exists.
- [x] On no-improvement, crash, or invalid result, revert `train.py` optimizer param-group change with `apply_patch` and return to `autoresearch/dev`.
- [x] Remove `run.log` and root `__pycache__` after metrics are captured.

## Code Changes
- **train.py**:
  - Build `decay_params` and `no_decay_params` from `model.named_parameters()`.
  - Send rank > 1 non-bias tensors to the `WEIGHT_DECAY` group.
  - Send 1D tensors and bias tensors to a `weight_decay=0.0` group.
  - Pass these groups to the existing `optim.SGD` call.

This tests targeted weight-decay exclusion for BatchNorm and bias parameters while preserving the main conv/linear `1e-4` weight decay that the current anchor uses.

## Configuration Changes
- Optimizer parameterization: single global weight decay -> two parameter groups.
- Main conv/linear weights: continue using `WEIGHT_DECAY = 1e-4`.
- BatchNorm scale/shift and bias parameters: use `weight_decay = 0.0`.
- `STAGE_WIDTHS`, `BATCH_SIZE`, `LR`, `MOMENTUM`, `LR_MILESTONES`, `MAX_STEPS`, `USE_CUDNN_BENCHMARK`, `USE_CHANNELS_LAST`, `USE_COMPILE`, seed, augmentation, optimizer class, and once-per-epoch validation remain unchanged.
...
- Log output: stdout and stderr captured in project-root `run.log`, then summarized into `.autoresearch/logs/exp-log-027.md` before cleanup.
...
- Treat the result as no-improvement if `Batches per epoch` is not 390, because the run did not preserve the intended batch size.
- Treat the result as no-improvement if the first LR drop does not occur at step 21000.
...
1. Verify baseline and threshold:
   ...
   - Pass condition: output reports `baseline=93.23`; the EXP-027 improvement threshold is `best_test_acc >= 93.33`.
   - Timeout: 30 seconds.
...
4. Verify validation cadence:
   ...
   - Timeout: 30 seconds.
5. Verify optimizer param groups:
   - Command: `rg -n "decay_params|no_decay_params|weight_decay" train.py`
   - Pass condition: output shows both planned groups, `weight_decay=WEIGHT_DECAY`, and `weight_decay=0.0`.
   - Timeout: 30 seconds.
6. Verify preserved batch size and schedule:
   - Command: `grep "Batches per epoch" run.log`; `grep "step 21000" run.log`.
   - Pass condition: batches per epoch is 390 and step 21000 includes `lr: 0.0100`.
   - Timeout: 30 seconds.
...
9. Verify hard constraints:
   ...
   - Pass condition: only the planned optimizer param-group diff was present during the run, fixed training budget was used, total wall-clock stayed under 10 minutes, and no protected files changed.
   - Timeout: 1 minute.
