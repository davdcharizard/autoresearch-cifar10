# Plan EXP-015: 28/56/112 First LR Drop at 23k
- **Created**: 2026-06-08
...
### Milestone 1: Implement isolated schedule retune
- [x] Change `LR_MILESTONES` in `train.py` from `[22000, 64000]` to `[23000, 64000]`.
...
- [x] Compare `best_test_acc` to the current 93.09% baseline with the +0.10 point margin; EXP-015 needs `best_test_acc >= 93.19%`.
- [x] Check whether the run reaches the planned first LR drop at 23k steps and retains enough LR 0.01 refinement time.
- [x] Remove temporary run logs after the result is captured in the experiment log and report.

## Code Changes
- **train.py**: change `LR_MILESTONES = [22000, 64000]` to `LR_MILESTONES = [23000, 64000]`.
- No architecture, depth, optimizer, loss, augmentation, precision, batch size, seed, dependency, or evaluation-harness changes.

## Configuration Changes
- `LR_MILESTONES`: `[22000, 64000]` -> `[23000, 64000]`. Rationale: EXP-014 showed the 28/56/112 model works with a 22k first drop and peaks late, but a slightly later first drop may preserve more high-LR exploration while still leaving roughly 11k post-drop steps under the observed step budget.
...
5. Check the primary metric condition:
   ...
   If the value is below `93.19`, classify EXP-015 as `no-improvement` even if it is above 93.09.
6. Schedule/throughput sanity review:
   inspect `run.log` for `lr: 0.0100`, `num_steps`, `num_epochs`, and `num_params`. The run should reach the first LR drop at step 23000; if it never reaches 23k, analysis should classify the result as undertrained or schedule-missed.
7. Scope review:
   `git diff -- train.py`
