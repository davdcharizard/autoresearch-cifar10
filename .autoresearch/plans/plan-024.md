# Plan EXP-024: Reachable Second LR Drop on 28/56/112 Anchor
- **Created**: 2026-06-08
...
### Milestone 1: Implement isolated second-milestone change
- [x] Create experiment branch `autoresearch/exp-024` from `autoresearch/dev`.
- [x] Modify only `train.py`.
- [x] Change `LR_MILESTONES` from `[21000, 64000]` to `[21000, 36000]`.
- [x] Preserve `STAGE_WIDTHS = (28, 56, 112)`, `WEIGHT_DECAY = 1e-4`, `BATCH_SIZE = 128`, FP32 training, channels-last, cuDNN benchmark, `torch.compile`, seed, augmentation, optimizer class, and once-per-epoch validation.
...
- [x] Confirm the first LR drop still occurs at step 21000 by checking `run.log` for `step 21000` and `lr: 0.0100`.
- [x] Confirm the second LR drop occurs at step 36000 by checking `run.log` for `step 36000` and `lr: 0.0010`.
...
- [x] Compare `best_test_acc` to the concrete EXP-024 threshold `93.33%` (`93.23 + 0.10`).
- [x] Record verdict and metrics in `.autoresearch/logs/exp-log-024.md`; the experiment index is updated during analysis.
- [ ] On improvement, commit `train.py`, merge into `autoresearch/dev`, and skip PR creation if no remote exists.
- [x] On no-improvement, crash, or invalid result, revert `train.py` schedule change with `apply_patch` and return to `autoresearch/dev`.
- [x] Remove `run.log` and root `__pycache__` after metrics are captured.

## Code Changes
- **train.py**: Change `LR_MILESTONES` from `[21000, 64000]` to `[21000, 36000]`. This preserves the validated first LR drop and tests whether a reachable late `lr=0.001` refinement phase improves the current anchor.

## Configuration Changes
- `LR_MILESTONES`: `[21000, 64000]` -> `[21000, 36000]`.
- `STAGE_WIDTHS`: unchanged at `(28, 56, 112)`.
- `WEIGHT_DECAY`: unchanged at `1e-4`.
- `BATCH_SIZE`, `LR`, `MOMENTUM`, `MAX_STEPS`, `USE_CUDNN_BENCHMARK`, `USE_CHANNELS_LAST`, `USE_COMPILE`, seed, augmentation, optimizer class, and once-per-epoch validation remain unchanged.

The prior failed second-drop experiment, EXP-003, moved the second milestone to 40k on the smaller FP32 ResNet-20 recipe. EXP-024 differs by preserving the validated 28/56/112 architecture and 21k first drop, then adding a short late second-drop phase after roughly 15k steps at `lr=0.01`.
...
- Log output: stdout and stderr captured in project-root `run.log`, then summarized into `.autoresearch/logs/exp-log-024.md` before cleanup.
...
- Treat the result as no-improvement if the first LR drop does not occur at step 21000, because the planned anchor recipe was not preserved.
- Treat the result as no-improvement if the second LR drop does not occur at step 36000, because the experiment did not actually test the chosen idea.
- Treat the result as no-improvement if the reachable second drop lowers the late plateau or final accuracy without improving `best_test_acc` beyond the required threshold.
...
1. Verify baseline and threshold:
   ...
   - Pass condition: output reports `baseline=93.23`; the EXP-024 improvement threshold is `best_test_acc >= 93.33`.
   - Timeout: 30 seconds.
...
4. Verify validation cadence:
   ...
   - Timeout: 30 seconds.
5. Verify anchor schedule:
   - Command: `grep "step 21000" run.log` and inspect that the same progress line includes `lr: 0.0100`; `grep "step 36000" run.log` and inspect that the same progress line includes `lr: 0.0010`.
   - Pass condition: both planned LR drops are reached during the run.
   - Timeout: 30 seconds.
...
8. Verify hard constraints:
   ...
   - Pass condition: only the planned `LR_MILESTONES` change was present during the run, fixed training budget was used, total wall-clock stayed under 10 minutes, and no protected files changed.
   - Timeout: 1 minute.
