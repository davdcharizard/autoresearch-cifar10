# Plan EXP-051: Partial Residual-Branch BN Scale Initialization
- **Created**: 2026-06-09
...
- [x] Create or check out `autoresearch/exp-051` from `autoresearch/dev`.
- [x] Modify `train.py` only: after `self.apply(self._weights_init)` in `ResNet.__init__`, initialize every `BasicBlock.bn2.weight` to `0.1`.
- [x] Verify the diff preserves model widths, batch size, optimizer, LR schedule, augmentation, label smoothing, compile, channels-last, and validation cadence.
...
- [x] Record final metrics and observations in `logs/exp-log-051.md`.

## Code Changes
- **`train.py`**: In `ResNet.__init__`, after normal Kaiming initialization, loop over `self.modules()` and for each `BasicBlock` call `init.constant_(m.bn2.weight, 0.1)`.
...
- Residual branch terminal BatchNorm scale initialization: default `1.0` -> `0.1`.
- Rationale: EXP-028 full zero-gamma (`0.0`) undertrained badly, but a nonzero partial residual scale may retain useful branch signal while testing a milder identity-bias mechanism.
