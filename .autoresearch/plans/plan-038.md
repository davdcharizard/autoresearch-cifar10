# Plan EXP-038: Increase Weight Decay to 2e-4
- **Created**: 2026-06-09
...
- [x] Create experiment branch `autoresearch/exp-038` from `autoresearch/dev`.
- [x] Modify only `train.py`.
- [x] Change `WEIGHT_DECAY = 1e-4` to `WEIGHT_DECAY = 2e-4`.
...
- [x] Confirm the stronger weight decay is present with `rg -n "WEIGHT_DECAY = 2e-4" train.py`.
...
- [x] Compare `best_test_acc` to the concrete EXP-038 threshold `93.80%` (`93.70 + 0.10`).
- [x] Record verdict and metrics in `.autoresearch/logs/exp-log-038.md`; the experiment index is updated during analysis.
- [x] On improvement, commit `train.py`, merge into `autoresearch/dev`, and skip PR creation if no remote exists.
- [ ] On no-improvement, crash, or invalid result, revert the `WEIGHT_DECAY` change with `apply_patch` and return to `autoresearch/dev`.
...
This tests whether stronger shrinkage improves generalization on the successful reflection-padding, label-smoothed 28/56/112 anchor.

## Configuration Changes
- `WEIGHT_DECAY`: `1e-4` -> `2e-4` (stronger L2 regularization; no throughput or schedule change expected).
...
- Log output: stdout and stderr captured in project-root `run.log`, then summarized into `.autoresearch/logs/exp-log-038.md` before cleanup.
...
1. Verify baseline and threshold:
   ...
   - Pass condition: output reports `baseline=93.70`; the EXP-038 improvement threshold is `best_test_acc >= 93.80`.
   - Timeout: 30 seconds.
...
4. Verify stronger weight decay, preserved anchor, and validation cadence:
   - Commands: `rg -n "WEIGHT_DECAY = 2e-4" train.py`, `rg -n "STAGE_WIDTHS = \\(28, 56, 112\\)|BATCH_SIZE = 128|LR_MILESTONES = \\[21000, 64000\\]|label_smoothing=0\\.05|RandomCrop\\(32, padding=4, padding_mode=\\\"reflect\\\"\\)" train.py`, and `rg -n "evaluator\\.evaluate|Eval\\(" train.py`
   - Pass condition: weight decay 2e-4 is present, the current anchor settings are preserved, one `Eval()` construction exists, and one epoch-level `evaluator.evaluate(...)` call exists.
   - Timeout: 30 seconds.
