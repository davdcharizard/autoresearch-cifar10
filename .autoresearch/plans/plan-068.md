...
- [x] Create experiment branch `autoresearch/exp-068` from `autoresearch/dev`.
- [x] Change only `train.py`, setting `CUTMIX_ALPHA = 2.0`.
...
- [x] Confirm startup markers in `run.log`: CUDA device, ResNet-20 parameter count, `CutMix alpha: 2.0, prob: 0.5, label smoothing: 0.05`, 300s budget, and 390 batches per epoch.
...
- [x] Update `logs/exp-log-068.md` with final metrics and verification results.

## Code Changes
- **train.py**: Change `CUTMIX_ALPHA` from `1.0` to `2.0`. This tests a smoother CutMix lambda distribution with fewer extreme patch-area samples while preserving the validated EXP-064 CutMix probability, endpoint smoothing, architecture, optimizer, LR schedule, augmentation, compile path, and validation cadence.

## Configuration Changes
- `CUTMIX_ALPHA`: `1.0` -> `2.0` (opposite alpha bracket after EXP-067 showed `alpha=0.5` did not beat the anchor).

## Execution Environment
- Method: local single-GPU foreground command from the project root.
- Resources: one available NVIDIA H20 class GPU; expected VRAM around the prior CutMix runs, roughly 660 MB peak.
...
- Abort if startup markers do not show `CUTMIX_ALPHA=2.0` and `CUTMIX_PROB=0.5`, or if the run is not using CUDA.
...
1. Confirm branch and diff:
   ...
   - Pass condition: branch is `autoresearch/exp-068`; tracked diff is limited to `train.py`; the only intended code change is `CUTMIX_ALPHA = 2.0`.
   - Timeout: 30 seconds.
...
4. Verify startup and scope markers:
   ...
   - Pass condition: CUDA device is used, ResNet-20 has 822,790 params, CutMix line reports `alpha: 2.0, prob: 0.5, label smoothing: 0.05`, time budget is 300s, and batches per epoch is 390.
   - Timeout: 30 seconds.
...
7. Compare against current baseline:
   - Baseline from experiment index before EXP-068: `94.11%`; required improvement threshold: `94.21%`.
