# Plan EXP-090: Fine Lower CutMix Probability p=0.4 on Spatial Anchor
- **Created**: 2026-06-10
...
- [x] Create branch `autoresearch/exp-090` from `autoresearch/dev`.
- [x] Modify only `train.py`.
- [x] Change `CUTMIX_PROB = 0.5` to `CUTMIX_PROB = 0.4`.
- [x] Keep `CUTMIX_ALPHA=1.0`, `CUTMIX_LABEL_SMOOTHING=0.05`, clean-batch label smoothing 0.05, reflection `RandomCrop` padding 3, `RandomHorizontalFlip(p=0.4)`, unit-std normalization, architecture, optimizer, LR milestones, batch size, seed, compile/channels-last, fixed 300s budget, and validation cadence unchanged.
...
- [x] Confirm startup lines include selected device, `RandomCrop padding: 3 reflect`, `RandomHorizontalFlip p: 0.4`, `ResNet-20 | params: 822,790`, `CutMix alpha: 1.0, prob: 0.4, label smoothing: 0.05`, and a 300s budget.
...
- [x] Record `best_test_acc`, `final_test_acc`, `final_test_loss`, `training_seconds`, `total_seconds`, `startup_seconds`, `peak_vram_mb`, `num_epochs`, `num_steps`, and `num_params` in `logs/exp-log-090.md`.
- [x] Classify as `improvement` only if `best_test_acc >= 94.61%`; classify smaller gains, ties, or regressions as `no-improvement`.

## Code Changes
- **train.py / hyperparameters**: Change only `CUTMIX_PROB` from `0.5` to `0.4`. This tests whether the padding-3 / flip-p=0.4 spatial anchor benefits from slightly less frequent regional mixed-label training while preserving the successful CutMix mechanism.

## Configuration Changes
- `CUTMIX_PROB`: `0.5` -> `0.4`.
- All other configuration remains unchanged: `CUTMIX_ALPHA=1.0`, `CUTMIX_LABEL_SMOOTHING=0.05`, clean-batch label smoothing 0.05, reflection crop padding 3, flip p=0.4, unit-std normalization, `STAGE_WIDTHS=(28, 56, 112)`, `WEIGHT_DECAY=2e-4`, `LR=0.1`, `LR_MILESTONES=[21000, 64000]`, batch size 128, FP32 compile/channels-last, seed 42, fixed 300s training budget, and once-per-epoch validation.

This plan intentionally re-enters the medium-importance CutMix probability family but differs from EXP-065 and EXP-066 in two ways: it uses the stronger EXP-085 spatial anchor, and it makes a fine lower-side move to p=0.4 rather than the broad failed p=0.25 bracket. It avoids the newly closed isolated crop/flip brackets, stronger scalar decay, alpha brackets, label-smoothing deviations, LR startup changes, schedule-only changes, batch-size deviations, policy augmentation, cutout, SE, downsampling smoothing, topology changes, and broad architecture changes.
...
- Log output: Capture stdout/stderr to `run.log`; write structured observations and final metrics to `.autoresearch/logs/exp-log-090.md`.
...
- Startup markers do not confirm `RandomCrop padding: 3 reflect`, `RandomHorizontalFlip p: 0.4`, and `CutMix alpha: 1.0, prob: 0.4, label smoothing: 0.05`.
...
3. Confirm implementation from code and log:
   ...
   - Pass condition: `CUTMIX_PROB` is `0.4`, startup log confirms `RandomCrop padding: 3 reflect`, `RandomHorizontalFlip p: 0.4`, and `CutMix alpha: 1.0, prob: 0.4, label smoothing: 0.05`.
