# Plan EXP-028: Zero-Initialize Residual Branch Last BatchNorm
- **Created**: 2026-06-08
...
### Milestone 1: Implement identity-preserving residual initialization
- [x] Create experiment branch `autoresearch/exp-028` from `autoresearch/dev`.
- [x] Modify only `train.py`.
- [x] In `ResNet.__init__`, after `self.apply(self._weights_init)`, iterate through `self.modules()` and set `init.constant_(m.bn2.weight, 0)` for each `BasicBlock` whose `bn2.weight` exists.
...
- [x] Confirm `git diff -- train.py` contains only the planned residual-branch initialization change.
- [x] Confirm validation cadence remains once per epoch with `rg -n "evaluator\\.evaluate|Eval\\(" train.py`.
- [x] Confirm the zero-init code is present with `rg -n "BasicBlock|bn2\\.weight|init\\.constant_" train.py`.
...
- [x] Compare `best_test_acc` to the concrete EXP-028 threshold `93.33%` (`93.23 + 0.10`).
- [x] Record verdict and metrics in `.autoresearch/logs/exp-log-028.md`; the experiment index is updated during analysis.
- [ ] On improvement, commit `train.py`, merge into `autoresearch/dev`, and skip PR creation if no remote exists.
- [x] On no-improvement, crash, or invalid result, revert `train.py` zero-initialization change with `apply_patch` and return to `autoresearch/dev`.
- [x] Remove `run.log` and root `__pycache__` after metrics are captured.

## Code Changes
- **train.py**:
  - Add a short post-initialization loop in `ResNet.__init__` after `self.apply(self._weights_init)`.
  - For every `BasicBlock`, set the second BatchNorm affine scale `bn2.weight` to zero with `init.constant_`.
  - Leave all other modules, hyperparameters, optimizer settings, augmentation, and evaluation calls unchanged.

This tests whether identity-preserving residual branch initialization improves optimization and generalization under the current fixed-budget 28/56/112 anchor.

## Configuration Changes
- Residual branch initialization: `BasicBlock.bn2.weight` default one initialization -> zero initialization after module construction.
- No hyperparameter values change.
- `STAGE_WIDTHS`, `BATCH_SIZE`, `LR`, `MOMENTUM`, `WEIGHT_DECAY`, `LR_MILESTONES`, `MAX_STEPS`, `USE_CUDNN_BENCHMARK`, `USE_CHANNELS_LAST`, `USE_COMPILE`, seed, augmentation, optimizer class, and once-per-epoch validation remain unchanged.
...
- Log output: stdout and stderr captured in project-root `run.log`, then summarized into `.autoresearch/logs/exp-log-028.md` before cleanup.
...
- Treat the result as invalid if `num_params` differs from `822,790`, because the experiment is intended to change initialization only.
...
1. Verify baseline and threshold:
   ...
   - Pass condition: output reports `baseline=93.23`; the EXP-028 improvement threshold is `best_test_acc >= 93.33`.
   - Timeout: 30 seconds.
...
4. Verify validation cadence:
   ...
   - Timeout: 30 seconds.
5. Verify residual zero-init implementation:
   - Command: `rg -n "BasicBlock|bn2\\.weight|init\\.constant_" train.py`
   - Pass condition: output shows a loop over `BasicBlock` modules and `init.constant_(m.bn2.weight, 0)`.
   - Timeout: 30 seconds.
...
9. Verify hard constraints:
   ...
   - Pass condition: only the planned initialization diff was present during the run, fixed training budget was used, total wall-clock stayed under 10 minutes, and no protected files changed.
   - Timeout: 1 minute.
