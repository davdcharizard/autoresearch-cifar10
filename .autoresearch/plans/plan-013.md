# Plan EXP-013: ResNet-20 Width 1.5x with Proven 24k First Drop
- **Created**: 2026-06-08
...
### Milestone 1: Implement isolated width increase
- [x] Change `STAGE_WIDTHS` in `train.py` from `(20, 40, 80)` to `(24, 48, 96)`.
- [x] Preserve `NUM_BLOCKS = 3`, `LR_MILESTONES = [24000, 64000]`, batch size, optimizer, augmentation, seed, FP32 precision, cuDNN benchmark, channels-last, `torch.compile`, and once-per-epoch evaluation.
- [x] Run syntax/lint checks and confirm the diff only touches the planned `STAGE_WIDTHS` constant in `train.py`.
...
- [x] Compare `best_test_acc` to the current 92.12% baseline with the +0.10 point margin; EXP-013 needs `best_test_acc >= 92.22%`.
- [x] Check whether the run reaches the planned first LR drop at 24k steps and retains enough LR 0.01 refinement time.
- [x] Remove temporary run logs after the result is captured in the experiment log and report.

## Code Changes
- **train.py**: change the architecture width constant from `STAGE_WIDTHS = (20, 40, 80)` to `STAGE_WIDTHS = (24, 48, 96)`. The model construction already derives all stage channel counts and the classifier input dimension from this tuple, so no wiring changes are expected.
- No depth, optimizer, loss, augmentation, precision, batch size, seed, dependency, scheduler, or evaluation-harness changes.

## Configuration Changes
- `STAGE_WIDTHS`: `(20, 40, 80)` -> `(24, 48, 96)`. Rationale: EXP-011 validated modest width scaling, and EXP-012 showed the 24k first drop is a better schedule anchor than moving earlier to 22k.
- `LR_MILESTONES`: unchanged at `[24000, 64000]`. Rationale: isolate the capacity increase and keep the successful EXP-011 first-drop timing.
...
5. Check the primary metric condition:
   ...
   If the value is below `92.22`, classify EXP-013 as `no-improvement` even if it is above 92.12.
6. Schedule/throughput sanity review:
   inspect `run.log` for `lr: 0.0100`, `num_steps`, `num_epochs`, and `num_params`. The run should reach the first LR drop at step 24000; if it never reaches 24k, analysis should classify the result as undertrained or schedule-missed.
7. Scope review:
   `git diff -- train.py`
   Pass condition: only the planned `STAGE_WIDTHS` change appears in `train.py`; `prepare.py`, dependency files, and evaluation harness are unchanged.
