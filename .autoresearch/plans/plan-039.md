# Plan EXP-039: Increase Weight Decay to 3e-4
- **Created**: 2026-06-09
...
### Milestone 1: Implement isolated stronger weight-decay scalar change
- [x] Create experiment branch `autoresearch/exp-039` from `autoresearch/dev`.
- [x] Modify only `train.py`.
- [x] Change `WEIGHT_DECAY = 2e-4` to `WEIGHT_DECAY = 3e-4`.
- [x] Preserve `STAGE_WIDTHS = (28, 56, 112)`, reflected `RandomCrop`, `BATCH_SIZE = 128`, `LR_MILESTONES = [21000, 64000]`, `LR = 0.1`, `MOMENTUM = 0.9`, `label_smoothing=0.05`, FP32 training, channels-last, cuDNN benchmark, `torch.compile`, seed, optimizer class, and once-per-epoch validation.
...
- [x] Confirm the stronger weight decay is present with `rg -n "WEIGHT_DECAY = 3e-4" train.py`.
- [x] Confirm the current anchor remains present with `rg -n "STAGE_WIDTHS = \\(28, 56, 112\\)|BATCH_SIZE = 128|LR_MILESTONES = \\[21000, 64000\\]|label_smoothing=0\\.05|RandomCrop\\(32, padding=4, padding_mode=\\\"reflect\\\"\\)" train.py`.
...
- [x] Compare `best_test_acc` to the concrete EXP-039 threshold `94.07%` (`93.97 + 0.10`).
- [x] Record verdict and metrics in `.autoresearch/logs/exp-log-039.md`; the experiment index is updated during analysis.
...
## Code Changes
- **train.py**:
  - Change only the optimizer weight-decay scalar from:
    `WEIGHT_DECAY = 2e-4`
    to:
    `WEIGHT_DECAY = 3e-4`.
  - Leave the schedule, batch size, architecture, augmentation, optimizer class, loss smoothing, fixed budget, seed, and evaluation path untouched.

This tests whether the stronger-weight-decay direction validated by EXP-038 continues to improve the late post-drop plateau.

## Configuration Changes
- `WEIGHT_DECAY`: `2e-4` -> `3e-4` (stronger L2 regularization; no throughput or schedule change expected).
- No model, augmentation, optimizer-class, label-smoothing, batch-size, learning-rate schedule, seed, dependency, or harness configuration changes.
...
- Log output: stdout and stderr captured in project-root `run.log`, then summarized into `.autoresearch/logs/exp-log-039.md` before cleanup.
...
1. Verify baseline and threshold:
   ...
   - Pass condition: output reports `baseline=93.97`; the EXP-039 improvement threshold is `best_test_acc >= 94.07`.
   - Timeout: 30 seconds.
...
4. Verify stronger weight decay, preserved anchor, and validation cadence:
   - Commands: `rg -n "WEIGHT_DECAY = 3e-4" train.py`, `rg -n "STAGE_WIDTHS = \\(28, 56, 112\\)|BATCH_SIZE = 128|LR_MILESTONES = \\[21000, 64000\\]|label_smoothing=0\\.05|RandomCrop\\(32, padding=4, padding_mode=\\\"reflect\\\"\\)" train.py`, and `rg -n "evaluator\\.evaluate|Eval\\(" train.py`
   - Pass condition: weight decay 3e-4 is present, the current anchor settings are preserved, one `Eval()` construction exists, and one epoch-level `evaluator.evaluate(...)` call exists.
   - Timeout: 30 seconds.
