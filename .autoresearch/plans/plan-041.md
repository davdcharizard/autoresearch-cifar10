# Plan EXP-041: Weight Decay 1.5e-4 Local Bracket
- **Created**: 2026-06-09
...
### Milestone 1: Implement isolated weight-decay scalar change
- [x] Create experiment branch `autoresearch/exp-041` from `autoresearch/dev`.
- [x] Modify only `train.py`.
- [x] Change `WEIGHT_DECAY = 2e-4` to `WEIGHT_DECAY = 1.5e-4`.
- [x] Preserve `STAGE_WIDTHS = (28, 56, 112)`, reflected `RandomCrop`, `BATCH_SIZE = 128`, `LR = 0.1`, `LR_MILESTONES = [21000, 64000]`, `MOMENTUM = 0.9`, `label_smoothing=0.05`, FP32 training, channels-last, cuDNN benchmark, `torch.compile`, seed, optimizer class, and once-per-epoch validation.
- [x] Run `python3 -m py_compile train.py`.
- [x] Run `uv run ruff check train.py`.
- [x] Confirm `git diff -- train.py` contains only the planned `WEIGHT_DECAY` scalar change.
- [x] Confirm the reduced weight decay is present with `rg -n "WEIGHT_DECAY = 1\\.5e-4" train.py`.
- [x] Confirm the current anchor remains present with `rg -n "STAGE_WIDTHS = \\(28, 56, 112\\)|BATCH_SIZE = 128|LR = 0\\.1|LR_MILESTONES = \\[21000, 64000\\]|label_smoothing=0\\.05|RandomCrop\\(32, padding=4, padding_mode=\\\"reflect\\\"\\)" train.py`.
...
- [x] Confirm the first LR drop occurs at step 21000 with `lr: 0.0100`, because LR remains `0.1`.
...
- [x] Compare `best_test_acc` to the concrete EXP-041 threshold `94.07%` (`93.97 + 0.10`).
- [x] Record verdict and metrics in `.autoresearch/logs/exp-log-041.md`; the experiment index is updated during analysis.
- [ ] On improvement, commit `train.py`, merge into `autoresearch/dev`, and skip PR creation if no remote exists.
- [x] On no-improvement, crash, or invalid result, revert the `WEIGHT_DECAY` change with `apply_patch` and return to `autoresearch/dev`.
- [x] Remove `run.log` and root `__pycache__` after metrics are captured and analyzed.

## Code Changes
- **train.py**:
  - Change only the weight-decay scalar from:
    `WEIGHT_DECAY = 2e-4`
    to:
    `WEIGHT_DECAY = 1.5e-4`.
  - Leave the architecture, augmentation, optimizer class, LR, LR milestones, batch size, loss smoothing, fixed budget, seed, and evaluation path untouched.

This tests whether the EXP-038 regularization gain can be improved by a slightly softer decay value inside the bracket bounded by the successful `2e-4` result and failed `3e-4` result.

## Configuration Changes
- `WEIGHT_DECAY`: `2e-4` -> `1.5e-4` (local midpoint below the current best stronger-decay anchor).
- No model, augmentation, optimizer-class, LR, label-smoothing, batch-size, milestone, seed, dependency, or harness configuration changes.
...
- Log output: stdout and stderr captured in project-root `run.log`, then summarized into `.autoresearch/logs/exp-log-041.md` before cleanup.
...
- Treat the result as invalid if `num_params` differs from `822,790`, because the experiment is intended to change only weight decay.
- Treat the result as no-improvement if `Batches per epoch` is not 390, if the first LR drop does not occur at step 21000, or if the post-drop LR is not `0.0100`.
...
1. Verify baseline and threshold:
   ...
   - Pass condition: output reports `baseline=93.97`; the EXP-041 improvement threshold is `best_test_acc >= 94.07`.
   - Timeout: 30 seconds.
...
4. Verify reduced weight decay, preserved anchor, and validation cadence:
   - Commands: `rg -n "WEIGHT_DECAY = 1\\.5e-4" train.py`, `rg -n "STAGE_WIDTHS = \\(28, 56, 112\\)|BATCH_SIZE = 128|LR = 0\\.1|LR_MILESTONES = \\[21000, 64000\\]|label_smoothing=0\\.05|RandomCrop\\(32, padding=4, padding_mode=\\\"reflect\\\"\\)" train.py`, and `rg -n "evaluator\\.evaluate|Eval\\(" train.py`
   - Pass condition: `WEIGHT_DECAY = 1.5e-4` is present, the current anchor settings are preserved, one `Eval()` construction exists, and one epoch-level `evaluator.evaluate(...)` call exists.
   - Timeout: 30 seconds.
...
8. Verify hard constraints:
   ...
   - Pass condition: only the planned weight-decay diff was present during the run, fixed training budget was used, total wall-clock stayed under 10 minutes, and no protected files changed.
   - Timeout: 1 minute.
