# Plan EXP-023: Lower Weight Decay on 28/56/112 Anchor
- **Created**: 2026-06-08
...
### Milestone 1: Implement isolated weight-decay change
- [x] Create experiment branch `autoresearch/exp-023` from `autoresearch/dev`.
- [x] Modify only `train.py`.
- [x] Change `WEIGHT_DECAY` from `1e-4` to `5e-5`.
- [x] Preserve `STAGE_WIDTHS = (28, 56, 112)` and `LR_MILESTONES = [21000, 64000]`.
- [x] Preserve `BATCH_SIZE = 128`, FP32 training, channels-last, cuDNN benchmark, `torch.compile`, seed, augmentation, and once-per-epoch validation.
- [x] Run `python3 -m py_compile train.py`.
- [x] Run `uv run ruff check train.py`.
- [x] Confirm `git diff -- train.py` contains only the planned weight-decay change.
...
- [x] Compare `best_test_acc` to the concrete EXP-023 threshold `93.33%` (`93.23 + 0.10`).
- [x] Record verdict and metrics in `.autoresearch/logs/exp-log-023.md`; the experiment index is updated during analysis.
- [ ] On improvement, commit `train.py`, merge into `autoresearch/dev`, and skip PR creation if no remote exists.
- [x] On no-improvement, crash, or invalid result, revert `train.py` weight-decay change with `apply_patch` and return to `autoresearch/dev`.
- [x] Remove `run.log` and root `__pycache__` after metrics are captured.

## Code Changes
- **train.py**: Change `WEIGHT_DECAY` from `1e-4` to `5e-5`. This isolates whether the widened 28/56/112 anchor is slightly over-regularized under the fixed 300s budget.

## Configuration Changes
- `WEIGHT_DECAY`: `1e-4` -> `5e-5` to reduce L2 regularization on the current best anchor.
- `STAGE_WIDTHS`: unchanged at `(28, 56, 112)`.
- `LR_MILESTONES`: unchanged at `[21000, 64000]`.
...
- Log output: stdout and stderr captured in project-root `run.log`, then summarized into `.autoresearch/logs/exp-log-023.md` before cleanup.
...
- Treat the result as no-improvement if lower weight decay raises final loss or late overfitting without improving `best_test_acc` beyond the required threshold.
...
1. Verify baseline and threshold:
   ...
   - Pass condition: output reports `baseline=93.23`; the EXP-023 improvement threshold is `best_test_acc >= 93.33`.
   - Timeout: 30 seconds.
...
5. Verify anchor schedule:
   - Command: `grep "step 21000" run.log` and inspect that the same progress line includes `lr: 0.0100`.
   - Pass condition: first LR drop is reached at step 21000 during the run.
   - Timeout: 30 seconds.
...
8. Verify hard constraints:
   ...
   - Pass condition: only the planned `WEIGHT_DECAY` change was present during the run, fixed training budget was used, total wall-clock stayed under 10 minutes, and no protected files changed.
   - Timeout: 1 minute.
