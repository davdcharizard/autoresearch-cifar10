# Plan EXP-036: Mild Batch Size 112 on Current Anchor
- **Created**: 2026-06-09
...
### Milestone 1: Implement isolated batch-size change
- [x] Create experiment branch `autoresearch/exp-036` from `autoresearch/dev`.
- [x] Modify only `train.py`.
- [x] Change `BATCH_SIZE = 128` to `BATCH_SIZE = 112`.
- [x] Preserve `STAGE_WIDTHS = (28, 56, 112)`, reflected `RandomCrop`, `label_smoothing=0.05`, `LR_MILESTONES = [21000, 64000]`, `LR = 0.1`, `MOMENTUM = 0.9`, `WEIGHT_DECAY = 1e-4`, FP32 training, channels-last, cuDNN benchmark, `torch.compile`, seed, optimizer class, and once-per-epoch validation.
- [x] Run `python3 -m py_compile train.py`.
- [x] Run `uv run ruff check train.py`.
- [x] Confirm `git diff -- train.py` contains only the planned `BATCH_SIZE` scalar change.
- [x] Confirm `BATCH_SIZE = 112` is present with `rg -n "BATCH_SIZE = 112" train.py`.
- [x] Confirm the current anchor remains present with `rg -n "STAGE_WIDTHS = \\(28, 56, 112\\)|LR_MILESTONES = \\[21000, 64000\\]|label_smoothing=0\\.05|RandomCrop\\(32, padding=4, padding_mode=\\\"reflect\\\"\\)" train.py`.
...
- [x] Confirm `Batches per epoch: 446` appears in `run.log`, reflecting `BATCH_SIZE = 112` with `drop_last=True`.
- [x] Confirm the first LR drop occurs at step 21000 with `lr: 0.0100`.
- [x] Confirm no second LR drop occurs before completion, because the preserved second milestone at 64000 should remain unreachable.
...
- [x] Compare `best_test_acc` to the concrete EXP-036 threshold `93.80%` (`93.70 + 0.10`).
- [x] Record verdict and metrics in `.autoresearch/logs/exp-log-036.md`; the experiment index is updated during analysis.
- [ ] On improvement, commit `train.py`, merge into `autoresearch/dev`, and skip PR creation if no remote exists.
- [x] On no-improvement, crash, or invalid result, revert the `BATCH_SIZE` change with `apply_patch` and return to `autoresearch/dev`.
- [x] Remove `run.log` and root `__pycache__` after metrics are captured and analyzed.

## Code Changes
- **train.py**:
  - Change only the batch-size constant from:
    `BATCH_SIZE = 128`
    to:
    `BATCH_SIZE = 112`.
  - Leave the first LR milestone at 21000 so the current label-smoothed reflection anchor schedule is preserved.
  - Leave the second milestone at 64000 so this experiment does not retry isolated second-drop retuning.
  - Leave the evaluation path untouched; `Eval.evaluate()` remains the ground-truth metric and still runs once per epoch.

This tests whether a milder smaller-batch stochasticity change improves the current anchor without the severe throughput loss observed at batch size 96.

## Configuration Changes
- `BATCH_SIZE`: `128` -> `112` (milder stochasticity and data-pass geometry change than EXP-025's failed batch size 96).
- No model, augmentation, optimizer, weight-decay, learning-rate schedule, label-smoothing, seed, dependency, or harness configuration changes.
...
- Log output: stdout and stderr captured in project-root `run.log`, then summarized into `.autoresearch/logs/exp-log-036.md` before cleanup.
...
- Treat the result as invalid if `num_params` differs from `822,790`, because the experiment is intended to change only batch size.
- Treat the result as no-improvement if `Batches per epoch` is not 446, if the first LR drop does not occur at step 21000, or if the unreachable second milestone at 64000 fires unexpectedly.
...
1. Verify baseline and threshold:
   ...
   - Pass condition: output reports `baseline=93.70`; the EXP-036 improvement threshold is `best_test_acc >= 93.80`.
   - Timeout: 30 seconds.
...
4. Verify batch size, preserved anchor, and validation cadence:
   - Commands: `rg -n "BATCH_SIZE = 112" train.py`, `rg -n "STAGE_WIDTHS = \\(28, 56, 112\\)|LR_MILESTONES = \\[21000, 64000\\]|label_smoothing=0\\.05|RandomCrop\\(32, padding=4, padding_mode=\\\"reflect\\\"\\)" train.py`, and `rg -n "evaluator\\.evaluate|Eval\\(" train.py`
   - Pass condition: batch size 112 is present, the current anchor settings are preserved, one `Eval()` construction exists, and one epoch-level `evaluator.evaluate(...)` call exists.
   - Timeout: 30 seconds.
5. Verify preserved schedule behavior, changed batch geometry, and parameter count:
   - Commands: `grep "Batches per epoch" run.log`; `grep "step 21000" run.log`; `grep "step 64000" run.log || true`; `grep "^num_params:" run.log`
   - Pass condition: batches per epoch is 446, step 21000 includes `lr: 0.0100`, step 64000 is absent, and `num_params` is `822,790`.
   - Timeout: 30 seconds.
...
8. Verify hard constraints:
   ...
   - Pass condition: only the planned batch-size diff was present during the run, fixed training budget was used, total wall-clock stayed under 10 minutes, and no protected files changed.
   - Timeout: 1 minute.
