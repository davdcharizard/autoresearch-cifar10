...
def gradient_centralization_inventory(model):
    ...
    for module_name, module in model.named_modules():
        if isinstance(module, nn.Conv2d):
            eligible.append((f"{module_name}.weight", module.weight))

    eligible_elements = sum(parameter.numel() for _, parameter in eligible)
    eligible_rows = sum(parameter.shape[0] for _, parameter in eligible)
    assert len(eligible) == 16
    assert eligible_elements == 2_742_704
    assert eligible_rows == 2_256
...
def new_gc_audit(device):
    ...
    audit = {
        ...
        "raw_removed": scalar(),
        "l2_removed": scalar(),
        "cross_removed": scalar(),
        "max_residual": scalar(),
        "nonfinite": scalar(),
        "samples": 0,
    }
    for phase in ("cutmix", "early_clean", "late_clean"):
        audit[f"{phase}_regularized"] = scalar()
        audit[f"{phase}_removed"] = scalar()
        audit[f"{phase}_samples"] = 0
    return audit
...
def regularize_and_centralize_gradients(
    ...
    if phase not in ("cutmix", "early_clean", "late_clean"):
        raise ValueError(f"Unknown GC phase: {phase}")

    all_gradients = [parameter.grad for parameter in all_parameters]
    eligible_gradients = [parameter.grad for _, parameter in eligible]
    ...
    if should_audit:
        audit["samples"] += 1
        audit[f"{phase}_samples"] += 1
        for gradient, row_mean, raw_row_mean in zip(
            eligible_gradients,
            row_means,
            raw_row_means,
            strict=True,
        ):
            elements_per_row = gradient.numel() // gradient.shape[0]
            regularized_energy = squared_l2_fp64(gradient)
            removed_energy = squared_l2_fp64(row_mean) * elements_per_row
            l2_row_mean = row_mean - raw_row_mean
            raw_removed = squared_l2_fp64(raw_row_mean) * elements_per_row
            l2_removed = squared_l2_fp64(l2_row_mean) * elements_per_row
            cross_removed = (
                2.0
                * (raw_row_mean.to(torch.float64) * l2_row_mean.to(torch.float64)).sum()
                * elements_per_row
            )
            audit["regularized"].add_(regularized_energy)
            audit["removed"].add_(removed_energy)
            audit["raw_removed"].add_(raw_removed)
            audit["l2_removed"].add_(l2_removed)
            audit["cross_removed"].add_(cross_removed)
            audit[f"{phase}_regularized"].add_(regularized_energy)
            audit[f"{phase}_removed"].add_(removed_energy)
    ...
    if should_audit:
        for gradient in eligible_gradients:
            ...
            post_mean = gradient.mean(
                dim=tuple(range(1, gradient.ndim)),
                keepdim=True,
            )
    ...
    for state in optimizer.state.values():
        for value in state.values():
            if torch.is_tensor(value):
                count.add_(
                    torch.count_nonzero(~torch.isfinite(value)).to(torch.float64)
                )
    return count
...
def main():
    ...
    eligible, eligible_elements, eligible_rows = gradient_centralization_inventory(
        model
    )
    ...
    assert excluded_elements == 6_186
    assert excluded_tensors == 28
    ...
    print(
        ...
        f"gc=regularized_direction_conv_only gc_audit_every={GC_AUDIT_EVERY} "
        f"effective_weight_decay={WEIGHT_DECAY} optimizer_weight_decay=0"
    )
    print(
        f"gc_inventory: tensors={len(eligible)} conv=16 classifier=0 "
    ...
    component_sum = (
        audit_values["raw_removed"]
        + audit_values["l2_removed"]
        + audit_values["cross_removed"]
    )
    component_abs_error = abs(removed_energy - component_sum)
    component_scale = max(
        abs(removed_energy),
        abs(audit_values["raw_removed"])
        + abs(audit_values["l2_removed"])
        + abs(audit_values["cross_removed"]),
        1e-12,
    )
    component_scaled_error = component_abs_error / component_scale
    component_diagnostic_pass = component_scaled_error <= 1e-5
    path_sum = gc_cutmix_steps + gc_early_clean_steps + gc_late_clean_steps
    phase_audit_sum = sum(
        audit_values[f"{phase}_samples"]
        for phase in ("cutmix", "early_clean", "late_clean")
    )
    phase_regularized_sum = sum(
        audit_values[f"{phase}_regularized"]
        for phase in ("cutmix", "early_clean", "late_clean")
    )
    phase_removed_sum = sum(
        audit_values[f"{phase}_removed"]
        for phase in ("cutmix", "early_clean", "late_clean")
    )
    phase_energy_error = max(
        abs(regularized_energy - phase_regularized_sum)
        / max(regularized_energy, torch.finfo(torch.float64).tiny),
        abs(removed_energy - phase_removed_sum)
        / max(removed_energy, torch.finfo(torch.float64).tiny),
    )
    gc_integrity = (
        gc_calls == step
        and gc_cutmix_steps == cutmix_applied_batches
        and gc_early_clean_steps == cutmix_eligible_batches - cutmix_applied_batches
        and gc_late_clean_steps == step - cutmix_eligible_batches
        and path_sum == step
        and audit_values["samples"] == 1 + (step - 1) // GC_AUDIT_EVERY
        and phase_audit_sum == audit_values["samples"]
        and math.isfinite(removed_energy)
        and removed_energy > 0.0
        and decomposition_error <= 1e-5
        and phase_energy_error <= 1e-12
    ...
    print(
        ...
        f"phase_audit_sum={phase_audit_sum}"
    )
    print(
    ...
    for phase in ("cutmix", "early_clean", "late_clean"):
        phase_regularized = audit_values[f"{phase}_regularized"]
        phase_removed = audit_values[f"{phase}_removed"]
        phase_norm_ratio = math.sqrt(
            phase_removed
            / max(phase_regularized, torch.finfo(torch.float64).tiny)
        )
        print(
            f"gc_energy_{phase}: samples={audit_values[f'{phase}_samples']} "
            f"regularized={phase_regularized:.12e} "
            f"removed={phase_removed:.12e} "
            f"removed_norm_ratio={phase_norm_ratio:.12e}"
        )
    print(
        f"gc_energy_components: raw_removed={audit_values['raw_removed']:.12e} "
        f"l2_removed={audit_values['l2_removed']:.12e} "
        f"cross_removed={audit_values['cross_removed']:.12e} "
        f"reconstructed={component_sum:.12e} "
        f"abs_error={component_abs_error:.12e} "
        f"scaled_error={component_scaled_error:.12e} "
        f"status={'PASS' if component_diagnostic_pass else 'ANOMALY'}"
    )
    print(
        f"gc_integrity: status={'PASS' if gc_integrity else 'FAIL'} "
        f"phase_energy_error={phase_energy_error:.12e} "
    ...
    if not gc_integrity:
        raise RuntimeError("Gradient Centralization integrity reconciliation failed")
    print("---")
